...
Inherits ReportClass
	...
	#tag Method, Flags = &h1
		Protected Function PrintTOCPages(Items() As String, MyPages As Group2D, Scale As Double = 1.0) As Boolean
		  ...
		  colwidth = ((PrintableWidth * 72 * Scale) - g.StringWidth("MM") / 2) // Two EM spacing between columns
		  ...
		  While CurrentItem < UBound(Items)
		    ...
		    If Left(Items(CurrentItem), 1) = "-" Then // New Category
		      ...
		      TempFont.Bold = False
		      TempHeight = TempHeight + GraphicsX.DrawFontStringV(g, Items(CurrentItem + 1), leftSide + Indent * Scale, runningBase + TempHeight, TempFont, colwidth, Scale, _
		      TempPage, "left", 0, "top")
		      //
		    ...
		    Else // Not a category change, just a regular item
		      TempHeight = GraphicsX.DrawFontStringV(g, Ltrim(Items(CurrentItem)), leftSide + (Indent * Scale), runningBase, TempFont, colwidth, Scale, _
		      TempPage, "left", 0, "top")
		      //
