...
Module SongML
	...
	#tag Method, Flags = &h21
		Private Sub CleanLyrics(ByRef lines() As String)
		  ...
		  // SongSelect uses undescores in music sheets to indicate a chord (or note) is played while the silabel is sung. We don't (yet).
	...
	#tag Method, Flags = &h1
		Protected Function Draw(songDoc As XmlDocument, g As Graphics, zoom As Double, StartPage As Integer = 1) As Group2D
		  ...
		  If lyrics.InStr(EndOfLine.UNIX + "---") <> 0 Then  // Multicolumn: flag and set up -- TODO: allow Multicolumn via page setup, not only explicit column break
		    Multicolumn = True
		  ...
		  For l = 1 To UBound(lines)
		    ...
		    If InStr(". 123456789", Left(line, 1)) > 0 Then
		      ...
		      Else
		        ...
		        lineCount = 0
		      End If
		      lineCount = lineCount + FindLyricLineCount(lines, l + lineCount, isBilingual, hasPrefix)
		      ElementHeight = Draw_ChordLyricLine(songDoc, g, x+leftEdge, runningBase, ColumnWidth, zoom, section, isBilingual, lines, l, chordPos, lineCount, Page)
		    ...
		    ElseIf Left(line, 1) = ";" Then
		      ' ---------------------- COMMENT ----------------------
		      ElementHeight = Draw_CommentLine(g, x+leftEdge, runningBase, ColumnWidth, zoom, line, Page)
		      LastLine = l
		    ...
		    ElseIf lines(l).Len = 0 Then
		      ...
		      LyricFont.OntoGraphics g, zoom
		      ElementHeight = SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/lyrics/@space_before") * zoom _
		      + g.TextHeight _
	...
	#tag Method, Flags = &h21
		Private Function Draw_ChordLyricLine(songDoc As XmlDocument, g As Graphics, x As Double, y As Double, width as Double, zoom As Double, section As String, isBilingual As Boolean, ByRef lines() As String, startLineIndex As Integer, chordPos() As Integer, lineCount As Integer, ByRef Page As Group2D) As Double
		  // this functions draws a chord line and the following lyrics lines that need to be spaced interdependent on the chord line
		  
		  // impoved spacing and hyphenation by SvA
		  ...
		  Dim highlightChorus, hasChords As Boolean
		  ...
		  bodyFont = SmartML.GetValueF(App.MyPrintSettings.DocumentElement, "lyrics")
		  bodyFont.OntoGraphics g, zoom
		  blankUnderChordWidth = g.StringWidth(blankUnderChord)
		  lineSpaceBefore = SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/lyrics/@space_before") * zoom
		  lineSpaceAfter = SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/lyrics/@space_after") * zoom
		  
		  If isBilingual Then
		    langFont = SmartML.GetValueF(App.MyPrintSettings.DocumentElement, "lyrics_language2")
		    langSpaceBefore = SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/lyrics_language2/@space_before") * zoom
		    langSpaceAfter  = SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/lyrics_language2/@space_after") * zoom
		  End If
		  If IsChorus(section) Then
		    highlightChorus = SmartML.GetValueB(App.MyPrintSettings.DocumentElement, "style/@highlight_chorus", False, True)
		  ...
		  While i <= lastLineIndex
		    If lines(i).Left(1) <> " " Then
		      x = x + Draw_LinePrefixes(g, x, y, zoom, lines, startLineIndex, lineCount, IsChorus(section), IsBilingual, printCapo, OutputGraphics)
		      Exit
		    End If
		    i = i + 1
		  Wend
		  ...
		  For i = 1 To UBound(chordPos)
		    ...
		    For j = startLineIndex To lastLineIndex
		      ...
		      If j = chordLineIndex Then
		        ...
		        slice = slice.RTrim
		        If slice.Len > 0 Then
		          chordFont.OntoGraphics g, zoom
		          sliceWidth(i-1) = g.StringWidth(slice + "M")  // 1 em spacing for chords
		          
		          If printCapo Then
		            slice = SingleTranspose(slice, capoPosition, useSharp)
		            capoFont.OntoGraphics g, zoom
		            sliceWidth(i-1) = Max(sliceWidth(i-1), g.StringWidth(slice + "M"))
		          End If
		        End If
		      Else
		        // lyrics
		        ...
		        If slice.Len > 0 Then
		          ...
		          If i > 1 And InStr(" _",line.Mid(st,1)) <> 0 Then // i=1 is text before the first chord
		            sliceWidth(i-1) = Max(sliceWidth(i-1), g.StringWidth(slice) + blankUnderChordWidth)
		          Else
		            sliceWidth(i-1) = Max(sliceWidth(i-1), g.StringWidth(slice ))
		          End If
		        End If
		      End If
		    Next
		  ...
		  If hasChords Then
		    ...
		    sliceX = lineX
		    sliceY = y + g.TextAscent  // Set baseline
		    For i = 1 To UBound(chordPos)
		      If chordPos(i) = 0 Then
		        slice = line.Mid(st)
		      Else
		      ...
		      st = chordPos(i)
		    Next
		    y = y + g.TextHeight + (SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/chords/@space_after") * zoom)
		    ...
		    If printCapo THEN
		      ...
		      For i = 1 To UBound(chordPos)
		        ...
		        st = chordPos(i)
		      Next
		      y = y + g.TextHeight + (SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/capo/@space_after") * zoom)
		    End If
		  End If
		  ...
		  For j = firstLineIndex To lastLineIndex
		    ...
		    For i = 1 To UBound(chordPos)
		      ...
		      offsetX = 0
		      If offsetC <> 0 Then
		        offsetX = g.StringWidth(StringUtils.RTrim(Line.Mid(st + offsetC, Abs(offsetC)), "_")) * Sign(offsetC)
		        offsetC = 0
		      End If
		      ...
		      If slice.Len > 0 Then
		        ...
		        If chordPos(i) > 2 Then
		          IF InStr(line.Mid(chordPos(i) + offsetC - 1, 2), "_") <> 0 Or (JoinedToNext And offsetC = 0) Then
		            dashStart = sliceX + offsetX + g.StringWidth(slice)
		          Else
		            dashStart = 0
		          End If
		        Else
	...
	#tag Method, Flags = &h21
		Private Function Draw_CommentLine(g As Graphics, x As Double, y As Double, Width as Double, zoom As Double, ByRef line As String, ByRef Page As Group2D) As Double
	...
	#tag Method, Flags = &h1
		Protected Function Draw_Footer(songDoc As XmlDocument, g As Graphics, x As Double, y As Double, width As Double, zoom As Double, PageNum As Integer, ByRef Page As Group2D) As Double
	...
	#tag Method, Flags = &h1
		Protected Function Draw_Header(songDoc As XmlDocument, g As Graphics, x As Double, y As Double, width As Double, zoom As Double, Page As Group2D) As Double
		  ...
		  If Len(songno) > 0 Then
		    songnoFont.OntoGraphics g, zoom
		    songnoWidth = g.StringWidth(songno) + EM_Width
		  ...
		  If len(author) > 0 Then
		    authorFont.OntoGraphics g, zoom
		  ...
		  Else
		    ...
		    Header.Append GraphicsX.DrawStringShapeV(title, x + (titleWidth / 2), y + height, titleFont, zoom) //title font has already been scaled
		    
		    'Author
		    authorFont.OntoGraphics g, zoom
		    authorWidth = authorWidth - EM_Width 'Will right justify if no song number, otherwise it's between the title and song number
		    Header.Append GraphicsX.DrawStringShapeV(author, x + width - songnoWidth - (authorWidth / 2), y + height, authorFont, zoom)
		    
		    'Songno
		    songnoFont.OntoGraphics g, zoom
		    songnoWidth = songnoWidth - EM_Width
		    Header.Append GraphicsX.DrawStringShapeV(songno, x + width - (songnoWidth / 2), y + height, songnoFont, zoom)
		    height = MaxHeight
		  ...
		  lineGap = Max(2, (g.TextHeight - g.TextAscent) / 4)
		  ...
		  Header.Append GraphicsX.DrawStringShapeV(ext, x + width - (g.StringWidth(ext) / 2), y + lineheight, extraFont, zoom)
		  'Presentation Order
	...
	#tag Method, Flags = &h21
		Private Function Draw_HeadingLine(g As Graphics, x As Double, y As Double, ColWidth As Double, zoom As Double, heading As String, Page As Group2D) As Double
		  ...
		  printHeaderBox = SmartML.GetValueB(App.MyPrintSettings.DocumentElement, "style/@header_adornment", True, True) '§ SvA ToDo : Add to Dialog
		  ...
		  ElseIf Uppercase(Left(heading, 1)) = "C" Then   ' -- CHORUS --
		    ...
		    Else
		      heading = Trim(heading)
		      If SmartML.GetValueB(App.MyPrintSettings.DocumentElement, "style/@highlight_chorus") Then
	...
	#tag Method, Flags = &h21
		Private Function Draw_LinePrefixes(g As Graphics, x As Double, y As Double, zoom As Double, ByRef lines() As String, startLineIndex As Integer, lineCount As Integer, isChorus As Boolean, IsBilingual As Boolean, printCapoChord As Boolean, ByRef Page As Group2D) As Double
		  Dim tempFont, langFont As FontFace
		  Dim i, j As Integer
		  Dim prefix As String
		  Dim lineTop, prefixWidth, nextLeft As Double
		  Dim cs As New CurveShape
		  Dim lineSpaceBefore, lineSpaceAfter, lineHeight, lineAscend As Double
		  Dim langSpaceBefore, langSpaceAfter, langHeight, langAscend As Double
		  ...
		  If lines(i).Left(1) = "." Then
		    ...
		    If printCapoChord Then
		      ...
		      y = y + g.TextHeight ' Skip past the capo chord line
		      y = y + (SmartML.GetValueN(App.MyPrintSettings.DocumentElement, "layout/capo/@space_before") * zoom)
		  ...
		  If IsBilingual Then
		    ...
		    End If
		    langFont.OntoGraphics g, zoom
		    langHeight = g.TextHeight
		    langAscend = g.TextAscent
		    If highlightChorus Then
		      langFont.Bold = Not langFont.Bold
		    End If
		  End If
		  ...
		  For j = i To startLineIndex + lineCount - 1
		    ...
		    If lineIs2ndLanguage Then
		      ...
		      Page.Append Draw_StringShape(x, lineTop + langAscend, prefix, langFont, zoom)
		      lineTop = lineTop + langHeight + langSpaceAfter
		      langFont.OntoGraphics g, zoom
		    Else
		      lineTop = lineTop + lineSpaceBefore
		      Page.Append Draw_StringShape(x, lineTop + lineAscend, prefix, tempFont, zoom)
		      lineTop = lineTop + lineHeight + lineSpaceAfter
		      If IsBilingual Then
		        tempFont.OntoGraphics g, zoom
		      End If
		    End If
		    prefixWidth = Max(prefixWidth, g.StringWidth(prefix))
		  Next j
		  ...
		  cs.x2 = nextleft
		  cs.y2 = linetop - g.TextHeight + g.TextAscent + Max(2, Round((g.TextHeight - g.TextAscent) / 4))
		    ...
		    For x = 1 To strlen
		      If Mid(lyrics, x, 1) = Chr(10) Then
		        ...
		        If Left(line, 1) = "[" Then
		          ...
		          section = section.Replace(" ",&u00A0) ' repace embedded spaces with &nbsp; so the constructed presentation order does not break
		          sectionAdded = False
		...
		Function ToHTML(songElement As XmlNode, exportOptions As HTMLExportOptions) As String
		  ...
		  For currLine = 1 To UBound(lines)
		    ...
		    ElseIf Left(lines(currLine), 1) = "[" Then
		      ...
		      isBilingual = sectionOptions <> Nil And sectionOptions.HasKey("L")
		      s = s + "  <p/><div class=""heading"">" + FullHeading(section, True).HTMLEntityEncode + "</div>" + EndOfLine
	...
	#tag Method, Flags = &h0
		Sub ToSetML(songElement As XmlNode, style As SlideStyle = Nil)
		  ...
		  htmlCode = ToHTML(songElement, htmlOptions)
