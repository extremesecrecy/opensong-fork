...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  HSizer.AddTop lbl_song_capo, .4, pge_contents
		  HSizer.AddTop pop_song_capo, .4, pge_contents
		  HSizer.AddTop pop_song_capo_accidentals, .4, pge_contents
		  HSizer.AddTop chk_song_capo_print, .4, pge_contents
		  HSizer.AddTop lbl_song_time_sig, .4, pge_contents
		  HSizer.AddTop pop_song_time_sig, .4, pge_contents
		  HSizer.AddTop lbl_song_tempo, .4, pge_contents
		  HSizer.AddTop pop_song_tempo, .4, pge_contents
		  HSizer.AddTop lbl_song_key, .4, pge_contents
		  HSizer.AddTop edt_song_key, .4, pge_contents
		  HSizer.AddTop lbl_song_aka, .4, pge_contents
		  HSizer.AddTop edt_song_aka, .4, pge_contents
		  HSizer.AddTop lbl_song_key_line, .4, pge_contents
		  HSizer.AddTop edt_song_key_line, .4, pge_contents
		  HSizer.AddTopHeight lbl_song_user1, .4, 0, pge_contents
		  HSizer.AddTopHeight edt_song_user1, .4, .1, pge_contents
		  HSizer.AddTopHeight lbl_song_user2, .5, 0, pge_contents
		  HSizer.AddTopHeight edt_song_user2, .5, .1, pge_contents
		  HSizer.AddTopHeight lbl_song_user3, .6, 0, pge_contents
		  HSizer.AddTopHeight edt_song_user3, .6, .1, pge_contents
	...
	#tag Method, Flags = &h0
		Function ActionInSetSave() As Boolean
		  ...
		  Case "image"
		    ...
		    Dim bgpath As String = App.GetDocumentFolderItem(App.BACKGROUNDS_FOLDER).NativePath
		    Dim osBgpath As String = App.DocsFolder.Child(App.STR_BACKGROUNDS).NativePath
		    ...
		    For i = 0 To lst_image_images.ListCount() - 1
		      ...
		      Dim f As FolderItem = img.GetImageFolderItem()
		      If (filename.StartsWith(bgpath) Or filename.StartsWith(osBgpath)) And _
		        (chk_image_store_as_link.Value Or ImageDefaults.LinkToBackgroundsFolder) Then
		        If f <> Nil Then
		          If filename.StartsWith(osBgpath) Then
		            SmartML.SetValueFI(xslide, "filename", f, SmartML.RelativePath.OpenSongBackgrounds)
		          Else
		            SmartML.SetValueFI(xslide, "filename", f, SmartML.RelativePath.SeparateBackgrounds)
		          End If
		        Else
		          If filename.StartsWith(osBgpath) Then
		            SmartML.SetValue(xslide, "filename", filename.Mid(osBgpath.Len() + 1))
		          Else
		      ...
		      ElseIf chk_image_store_as_link.Value And filename <> "" Then
		        If filename.StartsWith(docpath) Then
		          If f <> Nil Then
		            SmartML.SetValueFI(xslide, "filename", f, SmartML.RelativePath.OpenSongDocuments)
		          Else
		            SmartML.SetValue(xslide, "filename", "../" + filename.Mid(docpath.Len() + 1))
		          End If
		        Else
		          If f <> Nil Then
		            SmartML.SetValueFI(xslide, "filename", f)
		          Else
		      ...
		      If chk_external_embed_presentation.Enabled Then
		        ...
		        ElseIf f.Exists() Then
		          If chk_external_embed_presentation.Value Then
		            ...
		            SmartML.SetValueFI(xgroup, "@filename", f, SmartML.RelativePath.MyDocuments)
		            inputStream.Close
		      ...
		      Else
		        If f <> Nil Then
		          If App.IsPortable Then
		            SmartML.SetValueFI(xgroup, "@filename", f, SmartML.RelativePath.PortableRoot)
		          Else
		            SmartML.SetValueFI(xgroup, "@filename", f, SmartML.RelativePath.MyDocuments)
		          End If
		        Else
		      ...
		      If chk_external_embed_mediafile.Enabled Then
		        If f = Nil Then
		          If chk_external_embed_mediafile.Value Then
		            ...
		            Return False
		          Else
		            // we just ignore the fact that we do not have a FolderItem.
		            // It might be an URL or a MRL (Media Resource Locator, VLC's stream naming scheme)
		    ...
		    ElseIf btn_external_application.Stuck() Then
		      If rad_external_application_start.Value Then
		        ...
		        If f = Nil Then
		          ...
		          Return False
		        ElseIf f.Exists() Then
		          SmartML.SetValueFI(xgroup, "@app_filename", f)
		        Else
		          MsgBox(App.T.Translate("errors/fileutils/filenotfound", f.NativePath))
		          Return False
		        End If
		...
		Sub LoadSongFields()
		  ...
		  If Not IsNull(xbacks) Then
		    ...
		    For i = 0 To xbacks.ChildCount - 1
		      ...
		      Dim f As FolderItem = SmartML.GetValueFI(xbacks.Child(i), "filename")
		      ...
		      If imageIsLoaded Then
		        If f <> Nil Then
		          image.SetFilename(f.NativePath)
		        End If
		      Else
		        imageIsLoaded = image.SetImageFromFile(f)
		      End If
		...
		Protected Sub StoreSongFields()
		  ...
		  For i = 0 To lst_song_backgrounds.ListCount()-1
		    If lst_song_backgrounds.HasImage( i ) Then
		      ...
		      Dim imgFolderItem As FolderItem
		      If img <> Nil Then
		        imgFilename = img.GetImageFilename()
		        imgFolderItem = img.GetImageFolderItem()
		        bgpath = App.DocsFolder.Child("Backgrounds").NativePath
		      End If
		      
		      If chk_song_store_as_link.Value And imgFilename <> "" Then
		        If imgFolderItem <> Nil Then
		          If imgFilename.StartsWith(bgpath) Then
		            SmartML.SetValueFI(xback, "filename", imgFolderItem, SmartML.RelativePath.OpenSongBackgrounds)
		          ElseIf imgFilename.StartsWith(App.GetDocumentFolderItem(App.BACKGROUNDS_FOLDER).NativePath) Then
		            SmartML.SetValueFI(xback, "filename", imgFolderItem, SmartML.RelativePath.SeparateBackgrounds)
		          Else
		            SmartML.SetValueFI(xback, "filename", imgFolderItem, SmartML.RelativePath.Absolute)
		          End If
		        Else
		          If imgFilename.StartsWith(bgpath) And ImageDefaults.LinkToBackgroundsFolder Then
		            SmartML.SetValue xback, "filename", imgFilename.Mid(bgpath.Len+1)
		          Else
	...
	#tag Method, Flags = &h0
		Sub TranslateMe()
		  ...
		  pop_slide_transition.DeleteAllRows
		  For i = 0 To App.TransitionList.Ubound
		    pop_slide_transition.AddRow App.TransitionList(i)
		  Next i
		  ...
		  pop_song_time_sig.DeleteAllRows
		  For i = 0 To App.TimeSigList.Ubound
		    pop_song_time_sig.AddRow App.TimeSigList(i)
		  Next i
		  ...
		  pop_song_tempo.DeleteAllRows
		  For i = 0 To App.TempoList.Ubound
		    pop_song_tempo.AddRow App.TempoList(i)
		  Next i
	...
	#tag Method, Flags = &h0
		Sub UpdateMenuItems()
		  ...
		  If Status_SongOpen And Focus = edf_song_lyrics Then
		    Dim lines() As String
		    mnu_edit_interleave.Visible = True
		    mnu_edit_interleave_separator.Enabled = True
		    ...
		    App.DebugWriter.Write(CurrentMethodName + " set mnu_edit_interleave.Visible = True", 9)
		  Else
		    mnu_edit_interleave.Enabled = False
		    mnu_edit_interleave.Visible = False
		    mnu_edit_interleave_separator.Enabled = False
		    mnu_edit_interleave_separator.Visible = False
		    App.DebugWriter.Write(CurrentMethodName + " set mnu_edit_interleave.Visible = False", 9)
		  End If
		  App.DebugWriter.Write(CurrentMethodName + " mnu_edit_interleave Visible = " + IF(mnu_edit_interleave.Visible, "True", "False") + " Enabled = " + IF(mnu_edit_interleave.Enabled, "True", "False"), 9)
		...
		Sub Change()
		  ...
		  Case "image"
		    ...
		    For i = 0 To xslides.ChildCount - 1
		      ...
		      imageIsLoaded = False
		      f = SmartML.GetValueFI(xslides.Child(i), "filename")
		      imagestring = SmartML.GetValue(xslides.Child(i), "image")
		      If imagestring <> "" Then
		        imageIsLoaded = image.SetImageFromString(imagestring) 
		      End If
		      If Not imageIsLoaded Then
		        imageIsLoaded = image.SetImageFromFile(f)
		      End If
		  ...
		  Case "external"
		    ...
		    Case "presentation"
		      ...
		      f = SmartML.GetValueFI(xgroup, "@filename")
		      edt_external_presentation_file.Text = If(f = Nil, "", f.NativePath)
		    ...
		    Case "videolan"
		      ...
		      f = SmartML.GetValueFI(xgroup, "@filename")
		      edt_external_videolan_mediafilename.Text = If(f = Nil, "", f.NativePath)
