...
Protected Class SlideStyle
	...
	#tag Method, Flags = &h0
		Sub FromXML(xStyle As XmlNode)
		  ...
		  Dim bgFolderItem As FolderItem
		  imagestring = SmartML.GetValue(xstyle, "background", False)
		  If imagestring <> "" Then
		    Call Background.SetImageFromString(imagestring)
		    bgFolderItem = SmartML.GetValueFI(xstyle, "background/@filename", ImageDefaults.BackgroundsFolder)
		    If bgFolderItem = Nil Or Not bgFolderItem.Exists Or bgFolderItem.IsFolder Then
		      fileName = SmartML.GetValue(xstyle, "background/@filename", False)
		    Else
		      fileName = bgFolderItem.NativePath
		    End If
		    Background.SetFilename(fileName)
		  Else
		    bgFolderItem = SmartML.GetValueFI(xstyle, "background/@filename", ImageDefaults.BackgroundsFolder)
		    If bgFolderItem <> Nil And bgFolderItem.Exists And Not bgFolderItem.IsFolder Then
		      Call Background.SetImageFromFile(bgFolderItem)
		    Else
		      fileName = SmartML.GetValue(xstyle, "background/@filename", False)
	...
	#tag Method, Flags = &h0
		Function SecondLanguageFont() As FontFace
		  ...
		  f.Size = Floor(f.Size * MultilanguageSize/100)
		  f.ForeColor = MultilanguageColor
	...
	#tag Method, Flags = &h0
		Function ToXML() As XmlDocument
		  ...
		  End If
		  If ImageDefaults.LinkToBackgroundsFolder And Background.GetImageFilename().StartsWith(ImageDefaults.BackgroundsPath) Then
		    SmartML.SetValueFI(CurrChild, "@filename", Background.GetImageFolderItem, SmartML.RelativePath.SeparateBackgrounds)
		  Else
