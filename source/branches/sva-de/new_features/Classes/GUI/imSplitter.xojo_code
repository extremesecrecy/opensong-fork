...
Inherits Canvas
	...
	#tag Event
		Sub MouseDrag(X As Integer, Y As Integer)
		  ...
		  If IsHorizontalSplitter Then
		    ...
		    // ignore here if the mouse did not move
		    If dy <> 0 Then
		      MoveSplitter(dx,dy)
		    End If
		  Else
	...
	#tag Event
		Sub MouseUp(X As Integer, Y As Integer)
		  ...
		  If Me.IsDocked = False And isInDrag Then
		    
		    If Me.IsHorizontalSplitter Then
		      If Me.DockBefore And Me.Top < Me.DockBeforeSize Then
		        Dock(True)
		        Return
		      End If
		      
		      Dim ParentHeight As Integer = Me.Window.Height
		      If Me.DockAfter And Me.Top > (ParentHeight - DockAfterSize) Then
		        Dock(False)
		        Return
		      End If
		      
		    Else
		      If Me.DockBefore And Me.Left < Me.DockBeforeSize Then
		        Dock(True)
		        Return
		      End If
		      
		      Dim ParentWidth As Integer = Me.Window.Width
		      If Me.DockAfter And Me.Left > (ParentWidth - DockAfterSize) Then
		        Dock(False)
		        Return
		      End If
		...
		Sub Dock(before As Boolean)
		  ...
		  If Me.IsHorizontalSplitter Then
		    If before Then
		      ...
		      Me.PositionBeforeDock = If(isInDrag, startDragY, Me.Top)
		      
		    Else
		      // calculate distance of splitter to bottom of window
		      Me.PositionBeforeDock = Me.Window.Height - If(isInDrag, startDragY, Me.Top) - Me.Height
		    End If
		  Else
		    If before Then
		      // calculate distance of splitter to left side of window
		      Me.PositionBeforeDock = If(isInDrag, startDragX, Me.Left)
		    Else
		      // calculate distance of splitter to right side of window
		      Me.PositionBeforeDock = Me.Window.Width - If(isInDrag, startDragX, Me.Left)
		    End If  // before
		...
		Sub MoveSplitter(MoveX As Integer = 0, MoveY As Integer = 0)
		  ...
		  If Me.IsHorizontalSplitter Then
		    ...
		    If t < MinBeforeArea Then
		      dy = MinBeforeArea - Me.Top
		      t = MinBeforeArea
		    End If
		    
		    if dy <> 0 Then Me.Top = t
		  ...
		  Else
		    ...
		    If l < MinBeforeArea Then
		      dx = MinBeforeArea - Me.Left
		      l = MinBeforeArea
		    End If
		    
		    If dx <> 0 Then Me.Left = l
		  ...
		  If Me.mIsDocked Then
		    ...
		    before = mIsDockedPosition = imSplitterIs.DockedBefore
		    mIsDockedPosition = imSplitterIs.UnDocked
	...
	#tag Method, Flags = &h0
		Sub Undock()
		  ...
		  If Me.IsDocked Then
		    ...
		    // MoveSplitter might have changed the Docked status already
		    If mIsDocked Or mIsDockedPosition <> imSplitterIs.UnDocked Then
		      // Keep track of Docked status
		      Me.mIsDocked = False
	...
	#tag Note, Name = ReadMe
		imSplitter is Copyright © 2014-2021, Oliver Osswald, Switzerland, BASEL, and licensed under the MIT License.
	...
	#tag Property, Flags = &h21
		Private startDragX As Integer
	#tag EndProperty

	#tag Property, Flags = &h21
		Private startDragY As Integer
	#tag EndProperty
