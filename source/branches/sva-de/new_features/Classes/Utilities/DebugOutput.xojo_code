...
Protected Class DebugOutput
		  ...
		  If Level <= DebugLevel Then
		    #If DebugBuild Then
		      ...
		      If Level > 0 And Level <= 10 Then
		        levelText = NthField("none,fatal,critical,error,warning,notification,info,debug,trace_event,trace_call_stack,trace_detail", ",", Level+1)
		      Else
		        levelText = Str(Level)
		      End If
		      text = "{" + levelText + "} " + text
	...
	#tag Enum, Name = SeverityLevel, Type = Integer, Flags = &h0, Description = 5468697320697320686F77204920287376612D6465292075736520746865206C6576656C20706172616D6574657220696E205772697465
		fatal=1
		  critical
		  error
		  warning
