...
Implements IPreferences
		...
		Function GetValueFI(path As String, default As FolderItem = Nil, create As Boolean = False) As FolderItem
		  ...
		  If GetChild(path, pl, child) Then
		    #If RBVersion < 2019.02
		      f = SpecialFolder.Temporary.GetRelative(DecodeBase64(pl.GetString(child)))
		    #Else
		      f = FolderItem.FromSaveInfo(DecodeBase64(pl.GetString(child)))
		    #EndIf
		  End If
		  
		  If Error And ErrorNumber <> kErrMissingLeafNum Then
		    Return Nil
		  End If
		  ...
		  // If the saved Relative/SaveInfo data failed to produce a valid FolderItem,
		  // try to create one from the saved file path.
		  
		  If Left(child, 1) = "@" Then
		    pathChild = Mid(path, 2) + "-path"
		  Else
		    pathChild = path + "/path"
		  End If
		  
		  If GetChild(pathChild, pl, child) Then
		    If Error And ErrorNumber <> kErrMissingLeafNum Then
		      Return Nil
		    End If
		    If Not Error Then
		      Try
		        Dim filePath As String = pl.GetString(child)
		        If InStr(filePath, "//") > 0 Then
		          f = New FolderItem(filePath, FolderItem.PathTypeURL)
		        Else
		          f = New FolderItem(filePath, FolderItem.PathTypeAbsolute)
		        End If
		      End Try
		    End If
		  End If
		  ...
		  // Unable to get a valid FolderItem
		  
		  If create Then SetValueFI(path, default)
	...
	#tag Method, Flags = &h0
		Sub Remove(path As String)
		  // ToDo: the following code does not work. For now the node just stays
		  Dim elements() As String
		  Dim s, node As String
		  Dim pl As plistDict
		  
		  // Trap out any leading "/"
		  If path.StartsWith("/") Then path = path.Middle(1)
		  
		  elements = path.Split("/")
		  
		  If elements.Count = 0 Then // Pointing at root
		    Return
		  Else
		    node = elements.Pop
		    If Not node.StartsWith("@") Then
		      node = "@" + node
		    End If
		    elements.Append(node)
		    path = String.FromArray(elements, "/")
		  End If
		  
		  Call GetChild(path, pl, s)
		  If Not IsNull(pl) And Not plistobj.error Then
		    pl.Remove(s)
		  End If
	...
	#tag Method, Flags = &h0
		Sub SetValue(path As String, value As String)
		  ...
		  If Not Modified Then
		    If Not pl.Exists(child) Or pl.GetType(child) <> pl.kTypeString Or pl.GetString(child) <> value Then
		      Modified = True
		    End If
		  End If
	...
	#tag Method, Flags = &h0
		Sub SetValueB(path As String, value As Boolean)
		  ...
		  If Not Modified Then
		    If Not pl.Exists(child) Or pl.GetType(child) <> pl.kTypeBoolean Or pl.GetBoolean(child) <> value Then
		      Modified = True
		    End If
		  End If
	...
	#tag Method, Flags = &h0
		Sub SetValueC(path As String, value As Color)
		  ...
		  If Not Modified Then
		    If Not pl.Exists(child) Or pl.GetType(child) <> pl.kTypeString Then
		      Modified = True
		    Else
		      oldValue =  pl.GetString(child)
		    End If
		  End If
		  ...
		  If Not Modified Then
		    Modified = oldValue <> pl.GetString(child)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SetValueFI(path As String, value As FolderItem)
		  //++
		  // The AbsolutePath (pre-RB2006) or URLPath is to provide a human-readable
		  // description of the file pointed to by GetSaveInfo.  The
		  // application will try to use this value to create the FolderItem if the SaveInfo 
		  // does not yield a useable FolderItem
		  //--
		  ...
		  Dim oldValue, oldPath As String
		  If Not Modified Then
		    If Not pl.Exists(child) Or Not pl.Exists(pathchild) Or pl.GetType(child) <> pl.kTypeString Or pl.GetType(pathchild) <> pl.kTypeString Then
		      Modified = True
		    Else
		      oldValue = pl.GetString(child)
		      oldPath = pl.GetString(pathchild)
		    End If
		  End If
		  ...
		  If Not (Value Is Nil) Then
		    ...
		    #Else
		      pl.SetString(child, EncodeBase64(value.SaveInfo(SpecialFolder.Home)))
		    #EndIf
		  ...
		  If Not Modified Then
		    Modified = oldValue <>pl.GetString(child) Or oldPath <> pl.GetString(pathchild)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SetValueN(path As String, value As Double)
		  ...
		  If Not Modified Then
		    If Not pl.Exists(child) Then
		      Modified = True
		    Else
		      oldType = pl.GetType(child)
		      oldValue = pl.GetReal(child)
		    End If
		  End If
		  ...
		  If Not Modified Then
		    Modified = oldType <> pl.GetType(child) Or oldValue <>value
		  End If
		End Sub
	#tag EndMethod
