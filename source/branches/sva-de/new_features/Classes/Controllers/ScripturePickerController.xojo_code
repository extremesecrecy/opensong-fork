...
Implements ScriptureNotifier
	...
	#tag Method, Flags = &h1
		Protected Sub ChangeBible(newBible As String)
		  ...
		  If mCurrentBible <> Nil And mCurrentBible.Name = newBible Then Return
	...
	#tag Method, Flags = &h0
		Sub CommandNotification(command As String, sender As iScripturePicker, parameter As Variant)
		  ...
		  Case Else
		    ...
		    NotifyLiveMobility(False, False)
		  End Select
	...
	#tag Method, Flags = &h1
		Protected Function CompileSlideGroup() As XmlNode
		  ...
		  While currVerse <= UBound(verses)
		    ...
		    CurrentFromVerse + slideStartVerse, CurrentFromVerse + currVerse - 1)
	...
	#tag Method, Flags = &h1
		Protected Sub SetDefaultPassage()
		  If CurrentBible <> Nil Then
		    CurrentBible.GetDefaultPassage(CurrentBook, CurrentChapter, CurrentFromVerse, CurrentThruVerse)
		    If CurrentBible.ValidateCitation(CurrentBook, CurrentChapter, CurrentFromVerse, CurrentThruVerse) Then
		      Return
		    End If
		  End If
