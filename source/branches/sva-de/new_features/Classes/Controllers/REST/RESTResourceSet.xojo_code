...
Implements REST.RESTResource
		      ...
		      If slide_group.GetAttribute("type") = "song" Then
		        ...
		        If f <> Nil And f.Exists Then
		          ...
		          If Not IsNull(songDoc) then
		            slide.AppendChild(xml.SafeImportNode(songDoc.DocumentElement(), True))
		          End If
		        End If
	...
	#tag Method, Flags = &h21
		Private Function PresentSet(setName As String, slideIndex As Integer = 1, mode As Integer = PresentWindow.MODE_DUAL_SCREEN, countblanks As Boolean = False) As REST.RESTResponse
		  Dim result As REST.RESTresponse = Nil
		  Dim setXml As XmlDocument
		  ...
		  Else
		    ...
		    Else
		      ...
		      If MainWindow.LoadSet(setName) Then
		        Call MainWindow.ActionSetPresent(mode, slideIndex, countblanks)
		        result = New REST.RESTResponse("OK")
		      Else
		        result = New REST.RESTResponse("The requested action failed.", HttpStatus.InternalServerError)
		      End If
		  ...
		  case "load", _
		    ...
		    Case HttpMethod.Post
		      ...
		      Else
		        ...
		        case "present"
		          result = PresentSet(protocolHandler.Identifier(), protocolHandler.Parameter("slide", 0), protocolHandler.Parameter("display", 0), _
		          protocolHandler.Parameter("countblanks", False).BooleanValue)
