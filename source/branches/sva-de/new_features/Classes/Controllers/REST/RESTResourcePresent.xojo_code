...
Implements REST.RESTResource
	#tag Method, Flags = &h21
		Private Function GetSlide(identifier As String, responseFormat As String) As REST.RESTResponse
		  ...
		  Dim formats() As String = Array("native", "v2", "vmix")
		  If formats.IndexOf(responseFormat) < 0 Then
		    result = New REST.RESTresponse("Valid slide formats: " + Join(formats, ","), HttpStatus.BadRequest)
		    Return result
		  End If
		  ...
		  Else
		    ...
		    If identifier = "current" Then
		      slideIndex = PresentWindow.VisibleSlideIndex
		    End If
		    ...
		    If Not IsNull(xslide) Then
		      ...
		      Case "vmix"
		        //
		        // vmix format tries to stay in sync with what is actually showing in the presentation if "current" is requested
		        // the other formats will stay in sync with the slide that is selected regardless of presentation mode.
		        // vmix will return the requested slide regardless of mode if requested by index
		        //
		        Dim slides_slide As XmlNode = xml.DocumentElement.AppendChild(xml.CreateElement("slide"))
		        If PresentWindow.Mode = "F" Or PresentWindow.Mode = "N" Or identifier <> "current" Then
		          SmartML.CloneChildren xslide, slides_slide
		          SmartML.CloneAttributes xslide, slides_slide
		        End If
		      End Select
	...
	#tag Method, Flags = &h0
		Function GetStatus(reason As String = "") As REST.RESTResponse
		  Dim result As New REST.RESTResponse
		  Dim xml As XmlDocument
		  Dim root, present, screen, slide As XmlNode
		  
		  xml = result.CreateXmlResponse(Name(), "status", "", reason)
		  root = xml.DocumentElement()
		...
		Private Function HandleSlide(protocolHandler As REST.RESTProtocolHandler) As REST.RESTresponse
		  ...
		  Else
		    ...
		    ElseIf protocolHandler.Identifier() <> "" And _
		      ...
		      ElseIf IsNumeric(protocolHandler.Identifier()) Or protocolHandler.Identifier() = "current" Then
		        If protocolHandler.Method() = HttpMethod.Post Then
		          ...
		          End If
		        Else
		          result = GetSlide(protocolHandler.Identifier, protocolHandler.Parameter("format", "native").StringValue)
		        End If
