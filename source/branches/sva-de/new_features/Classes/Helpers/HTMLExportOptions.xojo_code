...
Protected Class HTMLExportOptions
	#tag Method, Flags = &h0
		Function Clone() As HTMLExportOptions
		  ...
		  newCopy.TargetStyleSheetName = TargetStyleSheetName
		  newCopy.HyphenateWordBreaks = HyphenateWordBreaks
	...
	#tag Method, Flags = &h0
		Sub Constructor()
		  ...
		  PrintStyle = False
		  UniqueFiles = True
		  TargetStyleSheetName = "style.css"
		  HyphenateWordBreaks = True
		  
		  mOutputFolder = Nil
		  
		  mStyleSheet = DefaultStyleSheet
	...
	#tag Method, Flags = &h0
		Sub CopyFrom(source As HTMLExportOptions)
		  ...
		  TargetStyleSheetName = source.TargetStyleSheetName
		  HyphenateWordBreaks = source.HyphenateWordBreaks
	...
	#tag Method, Flags = &h0
		Function DefaultStyleSheet() As FolderItem
		  Dim css As FolderItem
		  css = App.DocsFolder.Child(App.STR_SETTINGS).Child("style.css")
		  If css = Nil Or Not css.Exists Then
		    css = App.AppFolder.Child("OpenSong Settings").Child("style.css")
		    if not css.Exists Then css = Nil
		  End If
	...
	#tag Method, Flags = &h0
		Sub Load(prefs As IPreferences)
		  ...
		  HyphenateWordBreaks = prefs.GetValueB(kHyphenate, True, True)
		  ...
		  TargetStyleSheetName = prefs.GetValue(kTargetStyleSheetName, "style.css", True)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Save(prefs As IPreferences)
		  ...
		  prefs.SetValueB(kHyphenate, HyphenateWordBreaks)
		  prefs.SetValueFI(kFolder, mOutputFolder)
		  prefs.SetValueFI(kStyleSheet, mStyleSheet)
		  prefs.SetValue(kTargetStyleSheetName, TargetStyleSheetName)
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h0
		HyphenateWordBreaks As Boolean
	#tag EndProperty
	...
	#tag ComputedProperty, Flags = &h0
		#tag Note
			The stylesheet that is used as source if PrintStyle = False
		#tag EndNote
		#tag Getter
			Get
			  Return mStyleSheet
	...
	#tag Property, Flags = &h0
		#tag Note
			The name of the stylesheet in the targer folder
		#tag EndNote
		TargetStyleSheetName As String
	#tag EndProperty
	...
	#tag Constant, Name = kHyphenate, Type = String, Dynamic = False, Default = \"style/@hyphenate", Scope = Private
	#tag EndConstant
	...
	#tag Constant, Name = kTargetStyleSheetName, Type = String, Dynamic = False, Default = \"style/@target_name", Scope = Public
	#tag EndConstant
