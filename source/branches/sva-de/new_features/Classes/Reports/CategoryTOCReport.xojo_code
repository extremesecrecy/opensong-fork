...
Inherits TOCReport
	...
	#tag Method, Flags = &h0
		Function Generate(Zoom As Double = 1.0, MyPages As Group2D = Nil) As Boolean
		  ...
		  Dim i As Integer = MyPages.Count - 1
		  While i > -1
		    MyPages.Remove i
		    i = i - 1
		  Wend
		  ...
		  For i = 0 To songCount - 1
		    ...
		    If f <> Nil And f.Exists Then
		      ...
		      If curr <> Nil Then
		        ...
		        Next j
		      Else
		        App.DebugWriter.Write "CategoryTOCReport.ProcessSongs: Error loading " + f.NativePath
		        InputBox.Message App.T.Translate("errors/bad_format", f.NativePath, SmartML.ErrorString)
		      End If
		    End if
	...
	#tag Method, Flags = &h0
		Function ProcessThemes() As Boolean
		  ...
		  // Heapsort now has it's own ProgressWindow
