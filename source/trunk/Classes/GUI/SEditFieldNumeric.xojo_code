...
Inherits SEditField
	...
	#tag Event
		Sub LostFocus()
		  ...
		  Dim numText As String = StripDisplayFormat(Text).Trim  // IsNumeric is picky about blanks
		  
		  Dim ctrl As RectControl = Me.Window.Focus
		  App.DebugWriter.Write(CurrentMethodName + ": Control " + If(ctrl = Nil, "<Nil>", ctrl.Name) + " has focus on window", 7)
		  If Not IsNumeric(numText) Then
		    InputBox.Message(App.T.Translate("errors/number_please"))
		    SetValue InitialValue
		    If Not (Me.Window.Focus Is Me) Then
		      SetFocus
		    End If
		  Else
		    If dirty Then
		      CurrentValue = CDbl(numText)
		      If DisplayFormat.IndexOf("%") <> -1 Then CurrentValue = CurrentValue / 100
		      ValueToText
		      CurrentValue = CDbl(StripDisplayFormat(Text))  // now we changed it, round it according to DisplayFormat
		      If DisplayFormat.IndexOf("%") <> -1 Then CurrentValue = CurrentValue / 100
		    End If
	...
	#tag Method, Flags = &h0
		Function StripDisplayFormat(FieldText As String) As String
		  Dim s As String = Fieldtext.Trim
		  Dim fmt As String
		  
		  If s = "" Or DisplayFormat = "" Then Return s
		  Dim i, j, state As Integer
		  j = 0
		  state = 0
		  Do  // this "loop" acts as a block. Jump out of it in order to not (un)apply the format
		    For i = 1 To DisplayFormat.Length
		      If State = 2 Then  // quoted character
		        fmt = fmt + Mid(DisplayFormat, i, 1)
		        fmt = fmt.RTrim
		        state = 0
		      Else
		        Select Case Mid(DisplayFormat, i, 1)
		        ...
		        Case "0", "#", ",", ".", "-", "+", "E"
		          If state <> 1 Then
		            ...
		            state = 1
		          End If
		        Case "(", ")"  // if these occure we disregard the format completely
		          Exit Do
		        Case "%"
		          fmt = fmt + "%"
		          state = 0
		        Case " "
		          state = state  // ignore it and continue
		        Case Else
		          Exit Do
		        End Select
		      End If
		    ...
		    If fmt = "" or fmt = "#" Then Exit Do
		    
		    i = 1
		    j = 0
		    Dim f As String
		    Dim l As Integer = fmt.Length
		    Dim d, t As String
		    Dim ss As String
		    Dim nextJ As Boolean = True
		    Dim r As String = Format(9876.5, "#,###.0")
		    t = Mid(r, InStr(r, "9") + 1, 1)  // thousands separator
		    d = Mid(r, InStr(r, "6") + 1, 1)  // decimal separator
		    r = ""  // r is used as the extracted numerical part
		    
		    For i = 1 To s.Length
		      ss = Mid(s, i, 1)
		      If nextJ And ss <> " " Then
		        j = j + 1
		        if j > l Then
		          Exit Do
		        End If
	...
	#tag Method, Flags = &h0
		Function Value() As Double
		  ...
		  If dirty Then
		    ...
		    Dim currText As String = StripDisplayFormat(Text).Trim  // IsNumeric is picky about blanks
		    If IsNumeric(currText) Then
		      value = CDbl(currText)
		      If DisplayFormat <> "" Then
		        If DisplayFormat.IndexOf("%") <> -1 Then
		          value = value / 100
		        End If
