...
Inherits Canvas
	...
	#tag Method, Flags = &h21
		Private Function suggestWhetherControlShouldResize(c as rectControl) As Boolean
		  ...
		  // do what the previous lines did, but without introducing dependencies
		  // in case the controls are not actually used in the project
		  dim ti as Introspection.TypeInfo = Introspection.GetType(c)
		  while ti <> nil and ti.IsClass
		    if  ti.Name = "HTMLViewer" then return true
		    if  ti.Name = "SpriteSurface" then return true
		    if  ti.Name = "Rb3DSpace" then return true
		    if  ti.Name = "MoviePlayer" then return true
		    ti = ti.BaseType
		  wend
