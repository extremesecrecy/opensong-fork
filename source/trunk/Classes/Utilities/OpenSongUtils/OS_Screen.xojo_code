...
Protected Class OS_Screen
	#tag Method, Flags = &h0
		Function AvailableHeight() As Integer
		  #If TargetLinux
		    ...
		    'As fallback, we just subtract the non-available height according to Xojo from screen 0
		    Dim availableHeight As Integer = Height()
		    If m_Index = 0 Then
		      Dim s As Screen = Screen(0)
		      availableHeight = availableHeight - (s.Height - s.AvailableHeight)
		    End If
	...
	#tag Method, Flags = &h0
		Function AvailableWidth() As Integer
		  #If TargetLinux
		    'The determination of the available workspace region is unclear using Gdk/Gtk
		    'As fallback, we just subtract the non-available width according to Xojo from screen 0
		    Dim availableWidth As Integer = Width()
		    If m_Index = 0 Then
		      Dim s As Screen = Screen(0)
		      availableWidth = availableWidth - (s.Width - s.AvailableWidth)
		    End If
	...
	#tag Method, Flags = &h0
		Function Bounds() As REALbasic.Rect
		  Return New REALbasic.Rect(Me.Left, Me.Top, Me.Width, Me.Height)
		End Function
	#tag EndMethod
		...
		Sub Constructor(index as Integer)
		  ...
		  #If TargetLinux
		    ...
		    #Else
		      ...
		      Soft Declare Sub gdk_screen_get_monitor_geometry Lib "libgdk-3.so" (screen As Integer, monitor_num As Integer, ByRef Rect As GdkRectangle)
		    #EndIf
		    Try
	...
	#tag Method, Flags = &h0
		Sub Constructor(Screen As OS_Screen)
		  m_Index = Screen.m_Index
		  m_GtkScreen = Screen.m_GtkScreen
	...
	#tag Method, Flags = &h0
		Function ScaleFactor() As Double
		  #If RBVersion > 2018.03
		    Dim sf As Double = 1.0
		    Try
		      #If TargetLinux
		        Soft Declare Function gdk_screen_get_monitor_scale_factor Lib "libgdk-3.so" (screen As Integer, index As Integer) As Integer
		        sf = gdk_screen_get_monitor_scale_factor(m_GtkScreen, m_Index)
		      #Else
		        sf = Screen(m_Index).ScaleFactor
		      #EndIf
		    Finally
		      Return sf
		    End Try
