...
Protected Class Translator
	#tag Method, Flags = &h0
		Sub Constructor(f As FolderItem)
		  ...
		  If SmartML.ErrorCode <> 0 Then
		    mErrorText = SmartML.ErrorString
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function GetNode(path As String) As XmlNode
		  ...
		  App.DebugWriter.Write "Translator.GetNode: " + "language/" + path, 6
	...
	#tag Method, Flags = &h0
		Sub LoadPopup(path As String, popCont As PopupMenu, clear As Boolean = False)
		  //++
		  ...
		  Catch e As XmlException
		    App.DebugWriter.Write "Translator.LoadPopup: XML error looking for '" + path + "'", 2
		    App.DebugWriter.Write "Error is '" + RuntimeException(e).Message, 2
		    Return
		  End Try
		  If valueNodes.Length <> 0 And clear Then
		    popCont.DeleteAllRows
		  End If
	...
	#tag Method, Flags = &h1
		Protected Function ParseControlName(c As Control) As String
		  ...
		  If IsNumeric(parts(UBound(parts))) Then parts.Remove(UBound(parts))  // Val converted "info" to inf(inity) which is greater than 0 and thus was removed
	...
	#tag Method, Flags = &h0
		Function ParseHierarchicalName(tag As String, cont As RectControl) As String
		  ...
		  Dim contName, parentName As String
	...
	#tag Method, Flags = &h0
		Sub TranslateContainerControl(cc As ContainerControl, tag As String, fonts() As FontFace, doCaptions As Boolean, doFonts As Boolean)
		  Dim i As Integer
		  Dim cont As Control
		  ...
		  For i = 0 To cc.ControlCount - 1
		    cont = cc.Control(i)
		    App.DebugWriter.Write CurrentMethodName + ": On control " + cont.Name, 7
		    TranslateControl(cont, tag, fonts, doCaptions, doFonts)
		  Next i
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Sub TranslateContainerControl(ewc As EmbeddedWindowControl, tag As String, fonts() As FontFace, doCaptions As Boolean, doFonts As Boolean)
		  Dim i As Integer
		  Dim cont As Control
		  Dim cc As ContainerControl
		  
		  App.DebugWriter.Write CurrentMethodName + ": Begin translating conatinerControl " + ewc.Name + " with tag " + tag, 5
		  
		  If ewc.Window IsA TranslatableContainer Then
		    TranslatableContainer(ewc.Window).TranslateContainerControl(Self, ewc.Handle, tag, fonts, doCaptions, doFonts)
		  Else
		    cc = ewc.Container
		    If cc = Nil Then Exit
	...
	#tag Method, Flags = &h1
		Protected Sub TranslateControl(cont As Control, tag As String, fonts() As FontFace, doCaptions As Boolean, doFonts As Boolean)
		  Dim j As Integer
		  ...
		  ElseIf cont IsA GroupBox Then
		    ...
		    If doCaptions Then
		      name = tag + Mid(name, 5) + kAttributeCaption
		      txText = Translate(name, txFound)
		      If txFound Then
		  ...
		  ElseIf cont IsA CheckBox Then
		    ...
		    If doCaptions Then
		      name = ParseHierarchicalName(tag, checkCont) + kAttributeCaption
		      txText = Translate(name, txFound)
		      If txFound Then
		  ...
		  ElseIf cont IsA SButton Then
		    ...
		    If doCaptions Then
		      ...
		      If controlUsage = "btn" Then
		        setTooltip = Not sbuttonCont.Tooltip.IsEmpty
		        parts = Split(sbuttonCont.Name, "_")
		        parts.Remove 0
		        name = ParseHierarchicalName(tag, sbuttonCont)
		        temp = Translate(name + kAttributeCaption, txFound)
		        If txFound Then
		          If setTooltip Then
		            sbuttonCont.Tooltip = Translate(name)
		          End If
		        Else
		          // check if it is in shared
		          name = kTagShared + Join(parts, "_")
		          temp = Translate(name + kAttributeCaption, txFound)
		          If TxFound Then
		            If setTooltip Then
		              sbuttonCont.Tooltip = Translate(name)
		        ...
		        End If
		        If Not txFound Then
		          App.DebugWriter.Write(Chr(9) + "In TranslateWindow: Can't find " + ParseHierarchicalName(tag, sbuttonCont) + kAttributeCaption, 4)
		          temp = StringUtils.RTrim(old, " .")
		          txFound = True
		        End If
		        If txFound Then
		  ...
		  ElseIf cont IsA RadioButton Then
		    ...
		    If doCaptions Then
		      name = ParseHierarchicalName(tag, radioCont) + kAttributeCaption
		      txText = Translate(name, txFound)
		      If txFound Then
		    ...
		    If doCaptions Then
		      If listCont.ColumnCount > 1 Then
		        ...
		        For j = 0 To listCont.ColumnCount - 1
		          ...
		          Else
		            App.DebugWriter.Write(Chr(9) + "In TranslateWindow (ListBox): Can't find " + temp + kAttributeHead + Str(j+1), 4)
		          End If
		        Next j
		        txFound = True
		      End If
		  ...
		  ElseIf cont IsA TabPanel Then
		    ...
		    If doCaptions Then
		      ...
		      For j = 0 To tabCont.PanelCount - 1
		        ...
		        Else
		          App.DebugWriter.Write(Chr(9) + "In TranslateWindow (TabPanel): Can't find " + temp + kAttributeHead + Str(j+1), 4)
		        End If
		      Next j
		      txFound = True
		    End If
		    If doFonts And fonts(1).Name.Len > 0 Then
		      ...
		      #If TargetWindows And RBVersion > 2018.01 And RBVersion < 2019.01
		        ' #Pragma Warning "Workaround to set font for TabPanel...revisit at each new Xojo version (Feedback Case 54743)"
		        ' appears to have been fixed in Xojo 2019 r1
		        ' the following is a work around given in the feedback case above ...
		  ...
		  ElseIf cont IsA PushButton Then
		    ...
		    If doCaptions Then
		      ...
		      If controlUsage = "btn" Then
		        ...
		        End If
		        If Not txFound Then
		          App.DebugWriter.Write(Chr(9) + "In Translate (PushButton): Can't find " + ParseHierarchicalName(tag, buttonCont) + kAttributeCaption, 4)
		          temp = StringUtils.RTrim(old, " .")
		        End If
		        If old.Right(3) = "..." Then temp = temp + "…"
		  ...
		  ElseIf cont IsA Slider Then
		    sldCont = Slider(Cont)
		    If doCaptions Then
		      name = ParseHierarchicalName(tag, sldCont) + "/@helptag"
		      txText = Translate(name, txFound)
		      If txFound Then
		        sldCont.HelpTag = txText
		      End If
		      txFound = True  // don't report a missing helptag on a slider
		    End If
		    
		    
		  ElseIf cont IsA EmbeddedWindowControl Then
		    If Mid(name, 4, 1) = "_" Then
		      name = Mid(name, 5)
		    End If
		    TranslateContainerControl(EmbeddedWindowControl(cont), tag, fonts, doCaptions, doFonts)
		  End If
		  
		  If Not txFound Then
		    App.DebugWriter.Write(Chr(9) + "In TranslateWindow: Can't find " + name, 4)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub TranslateMenu(menuTag As String, menu As MenuItem)
		  ...
		  App.DebugWriter.Write CurrentMethodName + ": Begin translating Menu " + menu.Name + " with tag " + menuTag, 5
		  
		  numItems = menu.Count
		  
		  For i = 0 To numItems - 1
		    thisItem = menu.Item(i)
		    If thisItem.Text = menu.TextSeparator Then Continue
		    ...
		    If tfrom.Len > 1 Then ' skips bag text and separators
		      tto = Translate(menuTag + "/" + tfrom + "/@caption", txFound)
		      If Not txFound Then
		        tto = StringUtils.RTrim(menu.Item(i).Text, " .")
		      End If
	...
	#tag Method, Flags = &h0
		Sub TranslateWindow(win As Window, tag As String, fonts() As FontFace, doCaptions As Boolean = True, doFonts As Boolean = True)
		  ...
		  tag = tag + "/"
		  App.DebugWriter.Write CurrentMethodName + ": Begin translating window " + win.Title + " with tag " + tag, 5
		  
		  For i = 0 To win.ControlCount - 1
		    App.DebugWriter.Write CurrentMethodName + ": On control " + win.Control(i).Name, 7
		    cont = win.Control(i)
		    TranslateControl(cont, tag, fonts, doCaptions, doFonts)
		  Next i
