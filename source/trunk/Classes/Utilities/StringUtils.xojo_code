...
Module StringUtils
	...
	#tag Method, Flags = &h0
		Function CompareHymnBookOrder(s1 As string, s2 As string) As Integer
		  ...
		  //
		  If s1.Right(1) <> "/" Then s1 = s1 + "/"
		  If s2.Right(1) <> "/" Then s2 = s2 + "/"
		  //
		  ...
		  val1 = result1a.SubExpressionString(1).Val // Leading numbers
		  val2 = result2a.SubExpressionString(1).Val
		  ...
		  Dim strCompareResult As Integer
		  strCompareResult = s1.Compare(s2, ComparisonOptions.CaseInsensitive, App.LocaleForSort)
		  If strCompareResult <> 0 Then
	...
	#tag Method, Flags = &h0
		Function ContainsB(extends s As String, what As String) As Boolean
		  // Xojo 2022r2 added a String.Compare function.
		  ...
		  #Else
		    Return s.Contains(what, ComparisonOptions.Binary)
		  #EndIf
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target32Bit or Target64Bit))
		Function ContainsSU(extends s As String, what As String) As Boolean
		  // N.B. String.Compare() was added to the Xojo framework in version 2022r2.
	...
	#tag Method, Flags = &h1
		Protected Function CountRegEx(s As String, pattern As String) As Integer
		  ...
		  rm = re.Search( s )
		  while rm <> nil
		    'App.DebugWriter.Write rm.SubExpressionString(0) + " at " + str(rm.SubExpressionStartB(0)) + " matches " + pattern + " in " + s
		    out = out + 1
		    rm = re.Search
		  wend
	...
	#tag Method, Flags = &h0
		Function EndsWithSU(extends s As String, withWhat As String) As Boolean
		  // Return true if 's' ends with the string 'withWhat',
	...
	#tag Method, Flags = &h0
		Function ExtractString(s As String, match As RegExMatch, num As Integer) As String
		  ...
		  length = match.SubExpressionString(num).LenB
		  
		  result = MidB(s, start, length)
	...
	#tag Method, Flags = &h0
		Function NormalizeStringEncoding(Extends s As String, form As StringUtils.NormalizationForms = StringUtils.NormalizationForms.NFC) As String
		  //++
		  // MacOS stores filenames in NFD format, but most of OpenSong assumes that UTF-8 strings are in NFC format (and text in text fields is NFC)
		  //-- 
		  #If TargetMacOS Then
		    Return NormalizeStringEncodingMacOS(s, form)
		  #EndIf
		  
		  #If TargetWindows Then
		    Return NormalizeStringEncodingWindows(s, form)
		  #EndIf
		  
		  //++
		  // For the moment, Linux returns the string unchanged until the appropriate library can be investigated
		  //--
	...
	#tag Method, Flags = &h21
		Private Function NormalizeStringEncodingMacOS(s As String, form As StringUtils.NormalizationForms) As String
		  #If TargetMacOS
		    Dim formArgument As Integer
		    
		    Select Case form // Conversion table from https://developer.apple.com/documentation/corefoundation/cfstringnormalizationform
		    ...
		    If Not (s.Encoding = Nil) Then
		      ...
		      CFStringNormalize(mutableStringRef, formArgument)
		      s = mutableStringRef
	...
	#tag Method, Flags = &h21
		Private Function NormalizeStringEncodingWindows(s As String, form As StringUtils.NormalizationForms) As String
		  #If TargetWindows
		    ...
		    Const ERROR_INVALID_PARAMETER = &h57
		    Const ERROR_NO_UNICODE_TRANSLATION = &h459
		    Const ERROR_SUCCESS = 0
		    Const SIZEOF_WSTRING = 2
		    ...
		    Declare Function NormalizeString Lib "normaliz.dll" (NormForm As UInteger, lpSrcString As WString, cwSrcLength As Integer, lpDstString As Ptr, cwDstLength As Integer) As Integer
		    
		    Dim outLen As Integer
		    
		    // First call gets the length required for the string by setting the destination to Null and zero length
		    outLen = NormalizeString(formArgument, s, s.Length, Nil, 0)
		    If outLen <= 0 Then
		      Dim errorMessage As String
		      Select Case outLen
		      Case ERROR_INSUFFICIENT_BUFFER
		        errorMessage = "Insufficient buffer"
		      Case ERROR_INVALID_PARAMETER
		        errorMessage = "Invalid parameter"
		      Case ERROR_NO_UNICODE_TRANSLATION
		        errorMessage = "No Unicode translation"
		      Case ERROR_SUCCESS
		        errorMessage = "Success, but no results returned"
		      Case Else
		        errorMessage = "Undefined Windows error message number " + outLen.ToHex
		      End Select
		      App.DebugWriter.Write "Error converting " + s + " to Unicode form " + CStr(formArgument) + ", error is " + errorMessage, 1
		      Return s
		    End If
		    
		    Dim outBuf As New MemoryBlock((outLen * SIZEOF_WSTRING) + 1)
		    
		    outlen = NormalizeString(formArgument, s, s.Length, outBuf, outLen)
		    If outLen <= 0 Then
		      Dim errorMessage As String
		      Select Case outLen
		      Case ERROR_INSUFFICIENT_BUFFER
		      ...
		      Return s
		    End If
		    s = outBuf.WString(0).ConvertEncoding(Encodings.UTF8)
	...
	#tag Method, Flags = &h1
		Protected Function ReverseFields(FieldString As String, Separator As String) As string
		  Dim Fields() As String = Split(FieldString, Separator)
		  Dim fieldCount As Integer = UBound(Fields)
		  Dim s As String
		  For i As Integer = 0 To fieldCount \ 2
		    s = Fields(i)
		    Fields(i) = Fields(fieldCount - i)
		    Fields(fieldCount - i) = s
		  Next
	...
	#tag Method, Flags = &h1
		Protected Function Sprintf(src as string, ParamArray data as Variant) As string
		  ...
		  do until match = nil or index > UBound(data)
		    ...
		    replacement = replacement.ConvertEncoding(Encodings.UTF8)
		    start = match.SubExpressionStartB(0) + 1
		    length = LenB(match.SubExpressionString(0))
		    
		    src = LeftB(src, start - 1) + replacement + MidB(src, start + length)
		    
		    index = index + 1
		    start = match.SubExpressionStartB(0) + LenB(replacement)
	...
	#tag Enum, Name = NormalizationForms, Type = Integer, Flags = &h1, Description = 
		NFC
		  NFD
		  NFKC
		NFKD
