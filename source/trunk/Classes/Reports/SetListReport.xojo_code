...
Inherits LeadSheetReport
	#tag Method, Flags = &h0
		Function AddSet(SetName As String, Set As XmlDocument) As Boolean
		  ...
		  node = LeadSheetDoc.AppendChild(LeadSheetDoc.CreateElement("set_list"))
		  ...
		  While CurrentSetItem <> Nil
		    ...
		    ItemName = SetML.GetSlideGroupCaption(CurrentSetItem)
		    ...
		    If Left(ItemName, 2) <> "--" And _
		      ...
		      If InStr(ItemType, "scripture") > 0 Or InStr(ItemType, "custom") > 0 Then
		        ...
		        slidecount = 1
		        While slide <> Nil
		          slideBody = SmartML.GetValue(slide, "body", False)
		          slideBody = StringUtils.Trim(slideBody, StringUtils.WhiteSpaces)
		          Dim bodyLines() As String = Split(ReplaceLineEndings(slideBody, EndOfLine.UNIX), EndOfLine.UNIX)
		          For bodyLineCount As Integer = 0 To bodyLines.Ubound
		            If bodyLines(bodyLineCount).Len <> 0 Then
		              bodyLines(bodyLineCount) = " " + bodyLines(bodyLineCount)
		            End If
		          Next
		          If slidecount > 1 Then // Test for not the first slide
		            bodyLines.Append "-__" // Add a separator
		          End If
		          body = body + Join(bodyLines, EndOfLine.UNIX) + EndOfLine.UNIX
