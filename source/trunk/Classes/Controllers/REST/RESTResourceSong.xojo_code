...
Implements REST.RESTResource
	#tag Method, Flags = &h21
		Private Function GetSong(song As String, folder As String = "", format As String = "native") As REST.RESTResponse
		  Dim result As REST.RESTresponse = Nil
		  Dim xml As XmlDocument
		  Dim root As XmlNode
		  Dim formats() As String = Array("native", "set")
		  If formats.IndexOf(Format) < 0 Then
		    Return New REST.RESTResponse("Valid formats are: " + Join(formats, ","), HttpStatus.BadRequest)
		  End If
		  ...
		  If Not IsNull(songDoc) Then
		    ...
		    If Format = "set" Then
		      SongML.ToSetML(songDoc.DocumentElement)
		    End If
		    root.AppendChild(xml.SafeImportNode(songDoc.DocumentElement(), True))
	...
	#tag Method, Flags = &h0
		Function Process(ByRef restSocket As REST.RESTSocket, protocolHandler As REST.RESTProtocolHandler) As REST.RESTresponse
		  ...
		  folder = protocolHandler.Parameter("folder", "")
		  
		  Select Case protocolHandler.Action()
		  Case ""
		    If protocolHandler.Identifier().Len() = 0 Then
		      result = ListSongs(folder)
		    Else
		      result = GetSong(protocolHandler.Identifier, folder, protocolHandler.Parameter("format", "native"))
		    End If
		  ...
		  Case "present", _
		    ...
		    Case HttpMethod.Post
		      ...
		      Else
		        ...
		        case "load"
		          result = LoadSong(protocolHandler.Identifier, folder)
		          
		        Case "present"
		          result = PresentSong(protocolHandler.Identifier, folder, _
		  ...
		  Else
		    If protocolHandler.Identifier().Len() = 0 Then
		      result = GetSong(protocolHandler.Action, folder)
		    Else
		      result = New REST.RESTResponse("The requested action is not available.", HttpStatus.NotFound)
		    End If
		  End Select
