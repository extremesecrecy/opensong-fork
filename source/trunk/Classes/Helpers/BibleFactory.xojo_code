...
Protected Module BibleFactory
	#tag Method, Flags = &h1
		Protected Sub AddSearchFolder(searchFolder As FolderItem, build As Boolean = True)
		  ...
		  If IsValidBibleFolder(searchFolder) Then
		    ...
		    If mScriptureFolderIndex = -1 Then
		      mScriptureFolderIndex = mSearchFolders.UBound
		    End If
		    If build Then
		      BuildKnownBibles
		    End If
	...
	#tag Method, Flags = &h0
		Function BibleList(ReBuild As Boolean = False) As String()
		  //++
		  // This is a readonly property; do not implement the Set method
		  //
		  // Return a list of scripture files found in the current folder
		  //--
		  If ReBuild Or KnownBibleNames.LastIndex = -1 Then
		    BuildKnownBibles
		  End If
		  
		  Dim resultBibleList() As String
		  For Each bibleName As String In KnownBibleNames
		    resultBibleList.Append(bibleName)
		  Next bibleName
		  Return resultBibleList
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Sub BuildKnownBibles()
		  //++
		  // Remove all the existing Bibles since they belong to the old folder.
		  //--
		  Redim OpenedBibles(-1)
		  Redim KnownBibleNames(-1)
		  If KnownBiblesNameToFolderItem Is Nil Then
		    KnownBiblesNameToFolderItem = New Dictionary
		  Else
		    KnownBiblesNameToFolderItem.Clear
		  End If
		  ...
		  For j = 0 To mSearchFolders.Ubound
		    ...
		    For i = 1 To max
		      ...
		      If f.IsFileVisible And (Not f.Directory) Then
		        ...
		        End If
		        If BibleName <> "" And Not KnownBiblesNameToFolderItem.HasKey(BibleName) Then
		          KnownBibleNames.Append(BibleName)
		          KnownBiblesNameToFolderItem.Value(BibleName) = f
		        End If
		      End If
		    Next
		  Next
	...
	#tag Method, Flags = &h21
		Private Function DetermineBibleName(f As Folderitem) As String
		  ...
		  If KnownBiblesFolderItemToName = Nil Then 
		    KnownBiblesFolderItemToName = New Dictionary
		  End If
		  
		  If KnownBiblesFolderItemToName.HasKey(f.NativePath) Then
		    Return KnownBiblesFolderItemToName.Value(f.NativePath)
		  End If
		  ...
		  Try
		    ...
		    If xDoc.DocumentElement <> Nil Then
		      ...
		      End Select
		    End If
		    KnownBiblesFolderItemToName.Value(f.NativePath) = BibleName
		  Catch
		    // not a bible, skip
		  End Try
	...
	#tag Method, Flags = &h0
		Function GetBible(name As String) As iBible
		  ...
		  If Not KnownBiblesNameToFolderItem.HasKey(name) Then Return Nil
		  b = New Bible
		  If Not b.LoadBible(KnownBiblesNameToFolderItem.Value(name)) Then
		    Return Nil
		  End If
	...
	#tag Method, Flags = &h1
		Protected Sub RemoveSearchFolder(f As FolderItem)
		  If mSearchFolders.Ubound < 0 Then Return
		  Dim i As Integer
		  For i = mSearchFolders.Ubound DownTo 0
	...
	#tag Method, Flags = &h1
		Protected Function SearchFolders() As FolderItem()
		  // make a deep copy. We don't want to expose the privat property to modification
		  Dim folders() As FolderItem
		  Dim i As Integer
		  Redim folders(mSearchFolders.Ubound)
		  For i = 0 To mSearchFolders.Ubound
		    folders(i) = New FolderItem(mSearchFolders(i))
		  Next i
		  Return folders
		End Function
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private KnownBiblesFolderItemToName As Dictionary
	#tag EndProperty

	#tag Property, Flags = &h21
		Private KnownBiblesNameToFolderItem As Dictionary
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mScriptureFolderIndex As Integer
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mSearchFolders() As FolderItem
	#tag EndProperty
	...
	#tag ComputedProperty, Flags = &h1
		#tag Getter
			Get
			  If mScriptureFolderIndex < 0 Or mScriptureFolderIndex > mSearchFolders.Ubound Then
			    mScriptureFolderIndex = -1
			    Return Nil
			  End If
			  Return mSearchFolders(mScriptureFolderIndex)
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  RemoveSearchFolder(value)
			  If value = Nil Or Not IsValidBibleFolder(value) Then
			    mScriptureFolderIndex = -1
			  Else
			    mSearchFolders.Insert(0, value)
			    mScriptureFolderIndex = 0
			  End If
			  BuildKnownBibles
			End Set
		#tag EndSetter
