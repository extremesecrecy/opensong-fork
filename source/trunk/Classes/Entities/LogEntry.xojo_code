...
Protected Class LogEntry
	#tag Method, Flags = &h0
		Function AddLogEntry() As Boolean
		  ...
		  // entryNo is the number of the entry element If log=Nil this is equal to NumEntries, else it is take from log
		  
		  If ActivityLog <> Nil Then
		    NumEntries = ActivityLog.NumEntries + 1
		    entryNo = ActivityLog.LastEntryNo + 1
		    
		    ActivityLog.NumEntries = NumEntries
		    ActivityLog.LastEntryNo = entryNo
		  Else
		  ...
		  entry = head.AppendChild(xDoc.CreateElement(E_ENTRY + Str(entryNo)))
