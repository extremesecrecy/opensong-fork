...
Implements iPresentation
	...
	#tag Method, Flags = &h0
		Function PresenterNotes(slideIndex As Integer) As String
		  ...
		  If Not IsNull( m_oImpressDoc ) Then
		    Try
		      Dim oDrawPages As OLEObject = m_oImpressDoc.getDrawPages() 'com.sun.star.drawing.XDrawPages
		      If Not IsNull( oDrawPages ) Then
		        ...
		        If slideIndex > 0 and slideIndex <= oDrawPages.getCount() Then
		          ...
		          Dim oDrawPage As OLEObject = oDrawPages.getByIndex( slideIndex - 1 )
		          If Not IsNull( oDrawPage ) Then
		            Dim oNotesPage As OLEObject = oDrawPage.NotesPage
		            If Not IsNull(oNotesPage) Then
		              For i As Integer = 0 To oNotesPage.getCount - 1
		                If result <> "" Then
		                  result = result + EndOfLine
		                End If
		                Dim oShape As OLEObject = oNotesPage.getByIndex( i )
		                If oShape.supportsService("com.sun.star.drawing.TextShape") then
		                  result = result + oShape.String
		                End If
		              Next i
		            End If
		          End If
		    ...
		    If Not IsNull( oFrame ) Then
		      ...
		      Try
		        ...
		        If Not IsNull( oComponentWin ) Then
		          Dim oRectangle As OLEObject = oComponentWin.getPosSize()
		          Dim presWidth As Integer = oRectangle.Width
		          Dim presHeight As Integer = oRectangle.Height
		          If w <> presWidth Or h <> presHeight Then
		            Dim scale As Double = Min(presWidth / w, presHeight / h)
		            presWidth = presWidth * scale
		            presHeight = presHeight * scale
		            x = x + (w - presWidth) / 2
		            y = y + (h + presHeight) / 2
		            w = presWidth
		            h = presHeight
		      ...
		      If Not IsNull( oPresentation ) Then
		        ...
		        If Not IsShowing() Then
		          ...
		          Try
		            ...
		            #if TargetWin32
		              // translate Xojo's screen index to Window's monitor index
		              Dim scrn As OpenSongUtils.OS_Screen = OSScreen(m_presentScreen)
		              oPresentation.Display = scrn.Bounds.isOnMonitorIndex + 1  // The Display propert is 1-based
		            #else
		              oPresentation.Display = m_presentScreen + 1  // The Display propert is 1-based
		            #endif
		          Catch
		    ...
		    If Not IsNull( oPresentation ) Then
		      Try
		        ...
		        If Not IsNull( oPresController ) Then
		          ...
		          oPresController.gotoPreviousEffect()
		          result = True
