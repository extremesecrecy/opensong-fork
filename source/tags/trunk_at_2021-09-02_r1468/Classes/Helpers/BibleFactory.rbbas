...
Protected Module BibleFactory
	#tag Method, Flags = &h0
		Function BibleList() As String()
		  ...
		  If ScriptureFolder = Nil Then Return knownBibleNames
		      ...
		      Else
		        ...
		        Try
		          ...
		          If xDoc.DocumentElement <> Nil Then
		            ...
		            End Select
		          End If
		          If Not knownBibleFileNames.HasKey(BibleName) Then
		            knownBibleNames.Append(BibleName)
		            knownBibleFileNames.Value(BibleName) = f.Name
		          End If
		        Catch
		          // not a bible, skip
		        End Try
		      End If
		      If BibleName <> "" And Not knownBibleFileNames.HasKey(BibleName) Then
		        knownBibleNames.Append(BibleName)
		        knownBibleFileNames.Value(BibleName) = f.Name
		      End If
		    End If
	...
	#tag Method, Flags = &h0
		Function GetBible(name As String) As iBible
		  ...
		  If Not knownBibleFileNames.HasKey(name) Then Return Nil
		  b = New Bible
		  If Not b.LoadBible(ScriptureFolder.Child(knownBibleFileNames.Value(name))) Then
		    Return Nil
		  End If
			...
			Set
			  If value <> Nil Then
			    If value.Exists And value.IsFileVisible And _
			      ...
			      If knownBibleFileNames Is Nil Then
			        knownBibleFileNames = New Dictionary
			      Else
			        knownBibleFileNames.Clear
			      End If
			    End If
			  End If
			End Set
	...
	#tag Property, Flags = &h21
		Private knownBibleFileNames As Dictionary
	#tag EndProperty
