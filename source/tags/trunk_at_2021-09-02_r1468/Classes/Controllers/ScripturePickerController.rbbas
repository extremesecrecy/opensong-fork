...
Implements ScriptureNotifier
	...
	#tag Method, Flags = &h1
		Protected Sub CompileScriptureAndDisplay()
		  Dim activeSlide As XmlNode
		  Dim SlideList As XmlNodeList
		  Dim currentSlideVerseCount As Integer = 0
		  
		  NotifyDisableUI
		  If PassageThruVerse < PassageFromVerse Then
		    PassageFromVerse = CurrentFromVerse
		    PassageThruVerse = CurrentThruVerse
		  End If
		  If PassageSlideGroup Is Nil Then
		    PassageSlideGroup = CompileSlideGroup
		  End If
		  If PassageSlideGroup <> Nil Then
		    If PassageThruVerse < PassageFromVerse Then
		      activeSlide = SmartML.GetNode(PassageSlideGroup, "slides/slide")
		    Else
		      slideList = PassageSlideGroup.Xql("slides/slide[@startVerseIdx=" + Str(CurrentFromVerse) + "]")
		      If SlideList Is Nil Or SlideList.Length = 0 Then
		        activeSlide = Nil
		      Else
		        activeSlide = SlideList.Item(0)
		      End If
		      If activeSlide <> Nil Then
		        NotifyLiveDisplay(activeSlide)
		        currentSlideVerseCount = SmartML.GetValueN(activeSlide, "@VerseCount")
		      End If
		    End If
		  End If
		  If CurrentFromVerse + currentSlideVerseCount <= CurrentThruVerse Then
		    CurrentFromVerse = CurrentFromVerse + currentSlideVerseCount
		  End If
		  NotifyPassageChanged
		  NotifyEnableUI
		End Sub
	...
	#tag Method, Flags = &h1
		Protected Sub NotifyPassageChanged()
		  For Each o As iScripturePicker in Observers
		    o.ChangePassage(CurrentBook, CurrentChapter, CurrentFromVerse, CurrentThruVerse, PassageFromVerse, PassageThruVerse)
		  Next
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Sub SetDefaultPassage()
		  ...
		  CurrentFromVerse = 1
		  CurrentThruVerse = 1
		  If CurrentBible <> Nil Then
		    CurrentBible.SetDefaultPassage(CurrentBook,CurrentChapter,CurrentFromVerse,CurrentThruVerse)
		  End If
	...
	#tag Property, Flags = &h1
		#tag Note
			...
			If this is smaller then PasageThruVerse then no passage is beeing live diplayed
		#tag EndNote
