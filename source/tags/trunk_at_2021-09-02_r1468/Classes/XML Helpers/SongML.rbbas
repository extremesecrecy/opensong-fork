...
Module SongML
	...
	#tag Method, Flags = &h1
		Protected Function Draw_Footer(songDoc As XmlDocument, g As Graphics, x As Double, y As Double, width As Double, zoom As Double, PageNum As Integer, ByRef Page As Group2D, LayoutSettings As XmlDocument = Nil) As Double
		  ...
		  If PageNum > 1 Then
		    ...
		    End If
		  End If
		  Footer.Append GraphicsX.DrawLineV(x, y - height - 2, x + width, y - height - 2, RGB(0, 0, 0), g.PenWidth)
		  height = height + TextHeightV(tempFont, g, zoom) 'One line margin above horizontal line
		  Page.Append Footer
		  Return height
		End Function
	#tag EndMethod
		...
		Private Function Draw_HeadingLine(g As Graphics, x As Double, y As Double, ColWidth As Double, zoom As Double, heading As String, Page As Group2D, xLayout As XmlNode, literalVerseText As Boolean = False) As Double
		  ...
		  Case "V"  // -- VERSE --
		    If printHeaderBox Then
		      ...
		      rr.Width = TextWidthV(headerText, tempFont, g, zoom) + g.PenWidth + oddOffSet
		      rr.Height = tempHeight + g.PenWidth + oddOffSet
		      ...
		      HeadingLine.Append GraphicsX.DrawStringShapeV(headerText, rr.X, rry + tempAscent, tempFont, zoom)
		      // Position to new y
		      y = rry + rr.Height
		    Else
		  ...
		  Case "C"  // -- CHORUS --
		    If printHeaderBox Then
		      ...
		      MidX = x + (g.PenWidth / 2) + 1 + (TextWidthV(headerText, tempFont, g, zoom) / 2)
		      
		      HeadingLine.Append GraphicsX.DrawRectV(MidX, _
		      y + (g.PenWidth / 2) + (tempHeight + (1 * zoom)) / 2, TextWidthV(headerText, tempFont, g, zoom) + g.PenWidth + oddOffSet, tempHeight + (1 * zoom), _
		      g.ForeColor, g.PenWidth)
		      HeadingLine.Append GraphicsX.DrawStringShapeV(headerText, midx, y + g.PenWidth + tempAscent, tempFont, zoom)
		      
		      y = y + g.PenWidth + tempHeight + (3 * zoom)
		    Else
		  ...
		  Case "T"   // -- TAG --
		    If printHeaderBox Then
		      ...
		      y = y + tempHeight + g.PenHeight + (3 * zoom)
		    Else
		      HeadingLine.Append GraphicsX.DrawStringShapeV(headerText, x, y + tempAscent, tempFont, zoom, StringShape.Alignment.Left)
		      y = y + tempHeight
		    End If
		  ...
		  y = y + (SmartML.GetValueN(xLayout, "layout/sections/@space_after") * zoom)
		  Return y - oldY
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function TextAscentV(font As FontFace, g as Graphics, zoom As Double = 1) As Double
		  ...
		  ascent = g.TextAscent / amp
		  g.TextSize = Font.Size * zoom
		  Return ascent
		      ...
		      If (xbacks.ChildCount() > 0) Then
		        ...
		        For i As Integer = 0 To xlist.Length() - 1
		          ...
		          If SmartML.GetValueB(xbacks, "@link", False) = True And sfile <> "" Then
		            ...
		            Try
		              file = FileUtils.RelativePathToFolderItem(App.DocsFolder.Child("Backgrounds"), sfile)
		            Catch
		              file = Nil
		            End Try
