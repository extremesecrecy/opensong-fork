...
Protected Class Translator
	...
	#tag Method, Flags = &h0
		Sub TranslateMenu(menuTag As String, menu As MenuItem)
		  ...
		  For i = 0 To menu.Count - 1
		    If menu.Item(i).Text = menu.TextSeparator Then Continue
	...
	#tag Method, Flags = &h0
		Sub TranslateWindow(win As Window, tag As String, fonts() As FontFace, doCaptions As Boolean = True, doFonts As Boolean = True)
		  ...
		  tag = tag + "/"
		  App.DebugWriter.Write "Translator.TranslateWindow: Begin translating window " + win.Title + " with tag " + tag, 5
		  
		  For i = 0 To win.ControlCount - 1
		    App.DebugWriter.Write "Translator.TranslateWindow: On control " + win.Control(i).Name, 7
		    cont = win.Control(i)
		    ...
		    ElseIf cont IsA TabPanel Then
		      ...
		      If doFonts And fonts(1).Name.Len > 0 Then
		        ...
		        //- -
		        #If TargetWin32 And RBVersion > 2018.01
		          #Pragma Warning "Ignoring code to set font for TabPanel...revisit at each new Xojo version (Feedback Case 54743)"
		        #Else
