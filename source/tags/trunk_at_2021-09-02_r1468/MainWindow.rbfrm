...
#tag WindowCode
	...
	#tag Event
		Sub Open()
		  ...
		  HSizer.AddTop pop_song_capo_accidentals, .4
		...
		Function ActionInSetSave() As Boolean
		  ...
		  Case "image"
		    ...
		    For i = 0 To lst_image_images.ListCount()-1
		      ...
		      If filename <> "" And _
		        (chk_image_store_as_link.Value Or _
		        (ImageDefaults.LinkToBackgroundsFolder And img.GetImageFilename(True).StartsWith(ImageDefaults.BackgroundsPath))) _
		        Then
		  ...
		  Case "external"
		    If btn_external_presentation.GetStuck() Then
		      If chk_external_embed_presentation.Enabled Then
		        Dim f As FolderItem
		        If Trim(edt_external_presentation_file.Text) <> "" Then f = PathToFolderItem(edt_external_presentation_file.Text)
		        If f = Nil Then
		    ...
		    ElseIf btn_external_videolan.GetStuck() Then
		      ...
		      If chk_external_embed_mediafile.Enabled Then
		        ...
		        If Trim(edt_external_videolan_mediafilename.Text) = "" Then
		          ...
		          End If
		        Else
		          f = PathToFolderItem(edt_external_videolan_mediafilename.Text)
		        End If
		        If f = Nil Then
		  ...
		  Case Else
		    str = edt_slide_slides.Text.FormatUnixEndOfLine
	...
	#tag Method, Flags = &h1000
		Sub ActionSetAddSong()
		  ...
		  If Not CheckSongDB Then Return
		...
		Sub ActionSetDelete()
		  ...
		  If InputBox.Ask(App.T.Translate("questions/delete/@caption", pop_sets_sets.Text)) Then
		    ...
		    If App.DocsFolder.Child(App.STR_SETS).Child(pop_sets_sets.Text) <> Nil Then
		      fi = New FolderItem(App.DocsFolder)
		      For Each child As String In Array(App.STR_SETS, pop_sets_sets.Text)
		        If fi = Nil Or fi.LastErrorCode <> 0 Then Exit
		        fi = fi.Child(child)
		      Next
		      If fi <> Nil Then fi.Delete
		      If fi = Nil Or fi.LastErrorCode <> 0 Then
		        FileUtils.SetLastError(fi)
		        InputBox.Message FileUtils.LastError
		      End If
		    End If
		    If fi = Nil Or fi.Name <> pop_sets_sets.Text Or Not fi.Exists Then
	...
	#tag Method, Flags = &h0
		Sub ActionSetExport()
		  ...
		  // Instead of using folderDelimiter use the parent folder method to find the target directories that need to be built.
		  ...
		  If setFile = Nil Or NOT setFile.Exists Then
		    InputBox.Message App.T.Translate("errors/no_such_set", FileUtils.JoinNativePath(App.DocsFolder.NativePath, App.STR_SETS, CurrentSetName))
		    Return
		  End If
		  ...
		  While xgroup <> Nil
		    If SmartML.GetValue(xgroup, "@type") = "song" Then
		      ...
		      If song <> Nil And song.Exists Then
		        foundSong = True
		        presentation = SmartML.GetValue(xgroup, "@presentation", False)
		        rpt.AddSong(SmartML.GetValue(xgroup, "@path") + SmartML.GetValue(xgroup, "@name"),presentation)
		      Else
		        App.MouseCursor = Nil
		        PrintWindow.Hide
	...
	#tag Method, Flags = &h0
		Sub ActionSongAddLinked()
		  ...
		  songPath = Songs.DBPathFromFolderItem(f)
		  
		  'selected song cannot be current song
		  If songPath = lst_songs_songs.CellTag(lst_songs_songs.ListIndex, 0) + lst_songs_songs.Text Then
		  ...
		  For i = 0 To lst_song_linked.ListCount - 1
		    If songPath = lst_song_linked.List(i) Then
		      InputBox.Message(App.T.Translate("errors/linked_song/exists"))
		      Return
		    End If
		  Next i
		  
		  lst_song_linked.AddRow(songPath)
		...
		Sub ActionSongCopy()
		  ...
		  Else
		    ...
		    If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		      ...
		      Log.OldSongFileName = ConvertEncoding(oldfolder + lst_songs_songs.Text, Encodings.UTF8)
		      Log.SongFileName = ConvertEncoding(lst_songs_songs.CellTag(lst_songs_songs.ListIndex, 0) + lst_songs_songs.Text,Encodings.UTF8)
		      Log.DateAndTime = d
		  ...
		  If f <> Nil Then
		    ...
		    For i = 0 To lst_songs_songs.ListCount - 1
		      ...
		      If song <> Nil Then // silently skip non-XML files
		        If recursive Then
		          SongDbPath = lst_songs_songs.CellTag(i,0)
		...
		Sub ActionSongMove()
		  ...
		  Else
		    If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		      ...
		      // writeing that UTF16 string to an XML file truncates the result after the first 00 byte in the UTF16 byte stream
		      Log.Title = edt_song_title.Text
		      Log.Author = edt_song_author.Text
		      Log.CCLISongNumber = edt_song_ccli.Text
		      Log.OldSongFileName = ConvertEncoding(oldfolder + lst_songs_songs.Text,Encodings.UTF8)
		      Log.SongFileName = ConvertEncoding(folder + lst_songs_songs.Text,Encodings.UTF8)
		      Log.DateAndTime = d
		      Log.HasChords = Log.CheckLyricsForChords(edf_song_lyrics.Text)
		      Log.Moved = True
		  ...
		  If Len(t) > 0 Then
		    ...
		    If CheckSongDB Then
		      ...
		      If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		        ...
		        Log.Author = ""
		        Log.CCLISongNumber = ""
		        Log.SongFileName = ConvertEncoding(pop_songs_song_folders.Text + "/" + t, Encodings.UTF8)
		        Log.DateAndTime = d
		  ...
		  If Len(t) > 0 Then
		    ...
		    If Songs.RenameFile(edt_songs_curr_folder.Text + "/" + old, t) Then
		      ...
		      If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		        ...
		        Log.Author = edt_song_author.Text
		        Log.CCLISongNumber = edt_song_ccli.Text
		        Log.OldSongFileName = ConvertEncoding(edt_songs_curr_folder.Text + "/" + old, Encodings.UTF8)
		        Log.SongFileName = ConvertEncoding(lst_songs_songs.CellTag(lst_songs_songs.ListIndex, 0) + lst_songs_songs.Text, Encodings.UTF8)
		        Log.DateAndTime = d
		        Log.HasChords = Log.CheckLyricsForChords(edf_song_lyrics.Text)
		        Log.Renamed = True
		...
		Sub ActionSongSave(noWrite As Boolean = False, SaveOldSong As Boolean = False)
		  ...
		  If Not noWrite Then
		    ...
		    If App.CheckDocumentFolders(App.SONGS_FOLDER) = App.NO_FOLDER Then
		      If  App.DocsFolder <> Nil Then
		        InputBox.Message App.T.Translate("errors/create_songs_folder", FileUtils.JoinNativePath(App.DocsFolder.NativePath, App.STR_SONGS))
		      Else
		        InputBox.Message App.T.Translate("errors/no_docs_folder", "")
		      End If
		      noWrite = True
		  ...
		  If Not noWrite Then
		    ...
		    If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		      ...
		      Log.Author = edt_song_author.Text
		      Log.CCLISongNumber = edt_song_ccli.Text
		      Log.SongFileName = ConvertEncoding(lst_songs_songs.CellTag(Globals.OldSongSel, 0) + lst_songs_songs.Text, Encodings.UTF8)
	...
	#tag Method, Flags = &h0
		Function CheckSetDB() As Boolean
		  // this is used in many actions -> centralize it
		  // (there is no Sets FolderDB yet, so we just check the Sets folder)
		  
		  // Check if we have a sets folder if not offer to create one
		  If App.CheckDocumentFolders(App.SETS_FOLDER) = App.NO_FOLDER Then
		    If App.DocsFolder <> Nil Then
		      InputBox.Message App.T.Translate("errors/create_sets_folder", FileUtils.JoinNativePath(App.DocsFolder.NativePath, App.STR_SETS))
		    Else
		      MsgBox App.T.Translate("errors/no_docs_folder", "")
		    End If
		    Return False
	...
	#tag Method, Flags = &h0
		Function CheckSongDB() As Boolean
		  // this is used in many actions -> centralize it
		    ...
		    If SmartML.GetValue(slide_group, "@type", True) = "external" Then
		      ...
		      Case "presentation", "videolan"
		        ...
		        If Not IsNull(SmartML.GetNode(slide_group, "file")) Then
		          ...
		          Dim embedFile As FolderItem = GetFolderItem(SmartML.GetValue(slide_group, "@filename", False), FileUtils.PathType)
	...
	#tag Method, Flags = &h21
		Private Sub ColorizeBilingualSongtext()
		  ...
		  ScrollPosX = edf_song_lyrics.ScrollPositionX
		  ScrollPosY = edf_song_lyrics.ScrollPosition
		  hasFocus = edf_song_lyrics.Window.Focus = edf_song_lyrics
		  
		  Dim st As StyledText = edf_song_lyrics.StyledText
		  edf_song_lyrics.Visible = False  // does this reduce the flicker?
		  ...
		  edf_song_lyrics.ScrollPositionX = ScrollPosX
		  edf_song_lyrics.ScrollPosition = ScrollPosY
		  edf_song_lyrics.Visible = True
		  If hasFocus Then edf_song_lyrics.SetFocus
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub doWindowResize()
		  If HSizer <> Nil Then
		    ...
		    Const chk_song_capo_print_Width = 50
		    
		    HSizer.Resize
		    
		    width = lbl_song_aka.Width - chk_song_capo_print_Width
		    height = lbl_song_aka.Top - can_song_style.Top - chk_song_style.Height - 3
		    width = Min(width,Round(height * PreviewScreenRatio))
		    height = Max(Round(width / PreviewScreenRatio), lbl_song_capo.Top - can_song_style.Top - chk_song_style.Height - 3)
		    width = Round(height * PreviewScreenRatio)
		    ...
		    chk_song_capo_print.Width = Max(chk_song_capo_print_Width, can_song_style.Left - 12 - chk_song_capo_print.Left)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function LoadSong(f As FolderItem, updateFolderSelection As Boolean = False) As Boolean
		  ...
		  Dim fullpath, folderPath As String
		  Dim index As Integer = -1
		  ...
		  If f <> Nil And f.Exists Then
		    ...
		    folderPath = StringUtils.Chop(Songs.DBPathFromFolderItem(f.Parent), "/")
		    ...
		    If index > -1 Then
		      ...
		      If folderPath = "" Then
		        edt_songs_curr_folder.Text = Songs.GetFilterMain()
		      Else
		        edt_songs_curr_folder.Text = StringUtils.RTrim(folderPath, "/")
		      End If
	...
	#tag Method, Flags = &h0
		Sub LoadSongFields()
		  ...
		  SongML.LyricsToSections CurrentSong.DocumentElement, verseDict, order, sectionIsBilingual
		...
		Protected Sub StoreSongFields()
		  ...
		  For i = 0 To lst_song_backgrounds.ListCount()-1
		    If lst_song_backgrounds.HasImage( i ) Then
		      ...
		      If chk_song_store_as_link.Value And img.GetImageFilename(True) <> "" Then
	...
	#tag Method, Flags = &h0
		Sub TranslateMe()
		  ...
		  pop_song_capo_accidentals.AddRow App.T.Translate("general_song_editor/transpose/using/sharps/@caption")
		  pop_song_capo_accidentals.AddRow App.T.Translate("general_song_editor/transpose/using/flats/@caption")
	...
	#tag Method, Flags = &h0
		Sub UpdateMenuItems()
		  ...
		  Else // Sets tab
		    ...
		    Dim normalBGColor, readOnlyBGColor, disabledBGColor As Color
		    
		    normalBGColor = edt_song_title.BackColor
		    readOnlyBGColor = FillColor
		    disabledBGColor = FillColor
		    ...
		    If Status_InSetEditable Then
		      ...
		      // Custom/Scripture/Song slide
		      If Status_InSetSongEdit Then
		        edt_slide_name.Enabled = True
		        edt_slide_name.ReadOnly = True
		        edt_slide_name.BackColor = readOnlyBGColor
		        edt_slide_title.Enabled = True
		        edt_slide_title.ReadOnly = True
		        edt_slide_title.BackColor = readOnlyBGColor
		        edt_slide_subtitle.Enabled = True
		        edt_slide_subtitle.ReadOnly = True
		        edt_slide_subtitle.BackColor = readOnlyBGColor
		        edt_slide_slides.Enabled = True
		        edt_slide_slides.ReadOnly = True
		        edt_slide_slides.BackColor = readOnlyBGColor
		        edt_slide_notes.Enabled = True
		        edt_slide_notes.ReadOnly = False
		        edt_slide_order.Enabled = True
		        edt_slide_order.ReadOnly = False
		        edt_slide_order.BackColor = normalBGColor
		        edt_slide_origorder.Enabled = True
		        edt_slide_origorder.ReadOnly = True
		        edt_slide_origorder.BackColor = readOnlyBGColor
		        edt_slide_folder.Enabled = True
		        edt_slide_folder.ReadOnly = True
		        edt_slide_folder.BackColor = readOnlyBGColor
		      ...
		      Else
		        ...
		        edt_slide_name.BackColor = normalBGColor
		        edt_slide_title.Enabled = True
		        edt_slide_title.ReadOnly = False
		        edt_slide_title.BackColor = normalBGColor
		        edt_slide_subtitle.Enabled = True
		        edt_slide_subtitle.ReadOnly = False
		        edt_slide_subtitle.BackColor = normalBGColor
		        edt_slide_slides.Enabled = True
		        edt_slide_slides.ReadOnly = False
		        edt_slide_slides.BackColor = normalBGColor
		        edt_slide_notes.Enabled = True
		        edt_slide_notes.ReadOnly = False
		        ...
		        edt_slide_origorder.ReadOnly = False
		        edt_slide_origorder.BackColor = normalBGColor
		        edt_slide_folder.Enabled = False
		        edt_slide_folder.ReadOnly = False
		        edt_slide_folder.BackColor = normalBGColor
		      ...
		      edt_external_notes.Enabled = True
		    Else
		      ...
		      edt_external_notes.Enabled = False
...
#tag Events pop_songs_song_folders
	#tag Event
		Sub Change()
		  ...
		  Dim f As FolderItem
		  f = Songs.GetFile(Me.Text)
		  
		  If f = Nil Or Not f.Exists Then
		    Dim path As String
		    f = Songs.GetRootFolder
		    path = FileUtils.JoinNativePath(f.NativePath, ReplaceAll(Songs.CleanPath(Me.Text), "/", FileUtils.NativePathSeparator))
		    If InputBox.AskYN(App.T.Translate("questions/no_folder/@caption", path)) Then
		      f = songs.AddFolder(Me.Text)
		      If f = Nil And songs.ErrorCode <> 1 Then // Folder already exists
