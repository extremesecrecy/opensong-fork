...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Sub ReloadDirectory()
		  ...
		  Dim docsFolder As FolderItem
		  '++JRC If BackgroundFolder is Nil do nothing
		  If BackgroundFolder = Nil Then Return
		  
		  path = BackgroundFolder.NativePath
		  docsFolder = SpecialFolder.Documents
		  If docsFolder <> Nil And path.StartsWith(docsFolder.NativePath) Then
		    path = FileUtils.JoinNativePath(docsFolder.DisplayName, path.Mid(docsFolder.NativePath.Len))
		  End If
		  ...
		  If BackgroundFolder.NativePath <> App.DocsFolder.Child("Backgrounds").NativePath Then
		    ImageFiles.Append PathToFolderItem(m_parentName)
		    folderIndex = 1
		  End If
