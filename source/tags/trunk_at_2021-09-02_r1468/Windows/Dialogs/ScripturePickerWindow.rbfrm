...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub ChangePassage(bookNumber As Integer, chapterNumber As Integer, fromVerse As Integer, thruVerse As Integer, passageFromVerse As Integer, passageThruVerse As Integer)
		  ...
		  End If
		  SetLiveDisplayPassage(passageFromVerse, passageThruVerse)
	...
	#tag Method, Flags = &h0
		Sub SetLiveDisplayPassage(fromVerse As Integer, thruVerse As Integer)
		  Dim i As Integer
		  Dim oldFrom, oldThru As Integer
		  
		  
		  If thruVerse < fromVerse Or fromVerse < 1 Or thruVerse < 1 Then
		    thruVerse = -1
		    fromVerse = -1
		  Else
		    fromVerse = fromVerse - 1
		    thruVerse = thruVerse - 1
		  End If
		  If fromVerse = PassageLiveDisplayFromIdx And thruVerse = PassageLiveDisplayThruIdx Then
		    Return
		  Else
		    oldFrom = PassageLiveDisplayFromIdx
		    oldThru = PassageLiveDisplayThruIdx
		    PassageLiveDisplayFromIdx = fromVerse
		    PassageLiveDisplayThruIdx = thruVerse
		    
		    If oldThru = -1 Then
		      For i = fromVerse To thruVerse
		        lst_scr_from_verse.InvalidateCell(i, -1)
		      Next
		    ElseIf thruVerse = -1 Then
		      For i = oldFrom To OldThru
		        lst_scr_from_verse.InvalidateCell(i, -1)
		      Next
		    ElseIf oldThru < fromVerse -1 Or thruVerse < oldFrom - 1 Then
		      // we have two non-overlapping areas
		      For i = oldFrom To oldThru
		        lst_scr_from_verse.InvalidateCell(i, -1)
		      Next
		      For i = fromVerse To thruVerse
		      ...
		      Next
		    Else
		      For i = Min(oldFrom, fromVerse) To Max(oldThru, thruVerse)
		        lst_scr_from_verse.InvalidateCell(i, -1)
		      Next
		    End If
		  End If
	...
	#tag Property, Flags = &h1
		Protected PassageLiveDisplayFromIdx As Integer
	#tag EndProperty

	#tag Property, Flags = &h1
		Protected PassageLiveDisplayThruIdx As Integer
	#tag EndProperty
...
#tag Events btn_done
	#tag Event
		Sub Action()
		  ...
		  If edt_scr_characters.Value < 1 Or edt_scr_characters.Value > 1000 Then
		    CharsPerSlide(500)
		  End If
...
#tag Events pop_scr_version
	#tag Event
		Sub Change()
		  ...
		  SendCommand ScripturePickerController.cmdSelectBible, pop_scr_version.Text
		End Sub
	#tag EndEvent
#tag EndEvents
...
#tag Events lst_scr_from_verse
	...
	#tag Event
		Function CellBackgroundPaint(g as Graphics, row as Integer, column As Integer) As Boolean
		  ...
		  If row >= PassageLiveDisplayFromIdx And row <= PassageLiveDisplayThruIdx Then
		    g.ForeColor = RGB(256 - (256-HighlightColor.Red) * 0.5, 256 - (256-HighlightColor.Green) * 0.5, 256 - (256-HighlightColor.Blue) * 0.5)
		    g.AntiAlias = False
...
#tag Events edt_scr_characters
	#tag Event
		Sub LostFocus()
		  If Me.Value < Me.Minimum Or Me.Value > Me.Maximum Then
