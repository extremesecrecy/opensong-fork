...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub SetMode(mode As String, refresh As Boolean = True)
		  If mode = "B" Then
		    btn_mode_black.Stuck = True
		  Else
		    btn_mode_black.Stuck = False
		  End If
		  
		  If mode = "W" Then
		    btn_mode_white.Stuck = True
		  Else
		    btn_mode_white.Stuck = False
		  End If
		  
		  If mode = "H" Then
		    btn_mode_hidden.Stuck = True
		  Else
		    btn_mode_hidden.Stuck = False
		  End If
		  
		  If mode = "L" Then
		    btn_mode_logo.Stuck = True
		  Else
		    btn_mode_logo.Stuck = False
		  End If
		  
		  If mode = "F" Then
		    btn_mode_frozen.Stuck = True
		  Else
		    btn_mode_frozen.Stuck = False
		  End If
		  
		  If mode = "N" Then
		    btn_mode_normal.Stuck = True
		  Else
		    btn_mode_normal.Stuck = False
		  End If
