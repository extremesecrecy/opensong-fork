		...
		Function InsertSongIntoSet(song As Folderitem, atSlide As Integer, presentation As String, CheckLinked As Boolean, showErrorPopup As Boolean) As Boolean
		  ...
		  If atXSlide <> Nil Then
		    ...
		    While song <> Nil
		      ...
		      End If
		      Dim StyleIndex As String = SmartML.GetValue(xSongStyle, "@index")
		      If StyleIndex <> "" And Globals.Status_Presentation Then
		        Dim Style As SlideStyle
		        Try
		          Style = GetStyle(StyleIndex)
		        Catch e
		          If e IsA NilObjectException Or e IsA KeyNotFoundException Then
		            // ignore and use orignal xml style
		          Else
		            Raise e
		          End If
		        End Try
		        If Style <> Nil Then
		          xSongStyle = Style.ToXML.DocumentElement
		        End If
		      End If
