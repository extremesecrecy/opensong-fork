...
Inherits ReportClass
	...
	#tag Method, Flags = &h1
		Protected Function PrintTOCPages(Items() As String, MyPages As Group2D, Scale As Double = 1.0) As Boolean
		  ...
		  Dim Pic As Picture
		  Dim g As Graphics
		  Dim top, bottom, runningBase, leftSide As Double
		  ...
		  TempFont = SmartML.GetValueF(PrintSettings.DocumentElement, "lyrics")
		  TempFont.OntoGraphics g, Scale
		  
		  LineHeight = g.TextHeight
		  colwidth = (g.Width - g.StringWidth("MM")) / 2 // Two EM spacing between columns
		  ...
		  While CurrentItem < UBound(Items)
		    ...
		    If Left(Items(CurrentItem), 1) = "-" Then // New Category
		      ...
		      TempHeight = GraphicsX.DrawFontStringV(g, CurrentCategory, leftSide, runningBase, TempFont, colwidth, Scale, _
		      TempPage, CategoryAlign, 0, "top")
		      TempFont.Bold = False
		      TempHeight = TempHeight + GraphicsX.DrawFontStringV(g, Items(CurrentItem + 1), leftSide + (Indent * Scale), runningBase + TempHeight, TempFont, colwidth - (Indent * Scale), Scale, _
		      TempPage, "left", 0, "top", LineWrapIndent)
		      //
		      ...
		      If runningBase + TempHeight >= bottom Then
		        ...
		        TempFont.Bold = True
		        runningBase = runningBase + GraphicsX.DrawFontStringV(g, CurrentCategory, leftSide, runningBase, TempFont, colwidth, Scale, _
		    ...
		    Else // Not a category change, just a regular item
		      TempHeight = GraphicsX.DrawFontStringV(g, Ltrim(Items(CurrentItem)), leftSide + (Indent * Scale), runningBase, TempFont, colwidth - (Indent * Scale), Scale, _
		      TempPage, "left", 0, "top", LineWrapIndent)
		      //
		      ...
		      If runningBase + TempHeight >= bottom Then
		        ...
		        TempPage.Remove 0
		        TempFont.Bold = True
		        runningBase = runningBase + GraphicsX.DrawFontStringV(g, CurrentCategory + Continued, leftSide, runningBase, TempFont, colwidth, Scale, _
		        PageObj, CategoryAlign, 0, "top")
		        TempFont.Bold = False
		        //
