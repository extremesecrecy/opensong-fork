...
Protected Module SetML
	...
	#tag Method, Flags = &h1
		Protected Function GetSlide(xSet As XmlDocument, index As Integer) As XmlNode
		  ...
		  Exception ex As RuntimeException
		    App.DebugWriter.Write CurrentMethodName + ": Got a " + Introspection.GetType(ex).Name, 3
		    Return Nil
		...
		Function GetSong(slidegroup As XmlNode, Songs As FolderDB, ByRef songFolder As String) As XmlDocument
		  ...
		  If slidegroup <> Nil Then
		    ...
		    If Songfolder = "" Then
		      songfolder = "/"
		    End If
		    If songFolder.Right(1) <> "/" Then songFolder.Append("/")
	...
	#tag Method, Flags = &h21
		Private Function SplitLine(ByRef lines() As String, ByRef i As Integer, splitWidth As Integer, ByRef multiwrap As Boolean, ByRef smartWrapped As Boolean, ByRef lineCountPerLanguage() As Integer, ByRef MaxLineWidth As Integer, isBilingual As Boolean, ByRef MaxWidthLang As Boolean, g As Graphics, zoom As Double, bodyFont As FontFace, secondLanguageFont As FontFace, ByRef secondLanguage As Boolean, separationMark As String) As Integer
