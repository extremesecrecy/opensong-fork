...
Protected Module FileUtils
	#tag Method, Flags = &h0
		Function AbsolutePathOS(extends f As FolderItem) As String
		  //++
		  // AbsolutePath was deprecated in 2013r1 and removed completely in 2019r2.
		  // This extension to FolderItem will handle the change and allow for a mostly
	...
	#tag Method, Flags = &h1
		Protected Function IsReadonly(f As FolderItem) As Boolean
		  ...
		  Try
		    ...
		    tmp = f.Child("TMP" + Chr(&h41 + Xojo.Math.RandomInt(0, 25)) + CStr(Xojo.Math.RandomInt(1000, 9999)))
