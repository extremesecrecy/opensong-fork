#tag Window
Begin Window MainWindow Implements ScriptureReceiver,SongPickerReceiver
   BackColor       =   &c40404000
   Backdrop        =   0
   CloseButton     =   True
   Composite       =   False
   ...
   Begin SPagePanel pge_controls
      ...
      Begin Canvas can_controls_songs_mode
         ...
         Begin SEditField edt_songs_curr_folder
            ...
            Width           =   227
         End
         Begin SearchControl sch_songs_songs
            AutoDeactivate  =   True
            Enabled         =   True
...
#tag WindowCode
	...
	#tag Event
		Function CancelClose(appQuitting as Boolean) As Boolean
		  ...
		  // Cancel the close if we're presenting.z
	...
	#tag Event
		Sub Open()
		  ...
		  HSizer.AddWidth lbl_song_title, .5
		  HSizer.AddWidth edt_song_title, .5
		  HSizer.AddLeftWidth lbl_song_author, .5, .5
		  HSizer.AddLeftWidth edt_song_author, .5, .5
		  HSizer.AddLeftWidth lbl_song_copyright, .5, .5
		  HSizer.AddLeftWidth edt_song_copyright, .5, .5
		  HSizer.AddLeft lbl_song_ccli, .85
		  HSizer.AddLeft edt_song_ccli, .85
		  HSizer.AddWidth lbl_song_presentation, .5
		  HSizer.AddWidth edt_song_presentation, .5
	...
	#tag MenuHandler
		Function mnu_file_quit() As Boolean Handles mnu_file_quit.Action
			Quit
		End Function
	#tag EndMenuHandler
	...
	#tag Method, Flags = &h0
		Sub ActionInSetAddSlide(xslide As XmlNode)
		  If Status_SetOpen Then
		    ...
		    ' No need to UpdateMenuItems, since the following selection change will call it
		...
		Sub ActionSongMove()
		  ...
		  Else
		    ...
		    If App.MainPreferences.GetValueB(App.kActivityLog, True) And Globals.SongActivityLog <> Nil Then
		      ...
		      Log.Author = edt_song_author.Text
		      Log.CCLISongNumber = edt_song_ccli.Text
		      Log.OldSongFileName =  oldfolder + lst_songs_songs.Text
		      Log.SongFileName =  folder + lst_songs_songs.Text
		      Log.DateAndTime = d
		      Log.HasChords = Log.CheckLyricsForChords(edf_song_lyrics.Text)
		      Log.Moved = True
	...
	#tag Method, Flags = &h21
		Private Sub DoSongSearch(searchText As String)
		  If SavedSongList.Ubound < 0 Then
		  ...
		  Redim SearchSongList(-1)
		  lst_songs_songs.RemoveAllRows
		  
		  For Each song As Pair In SavedSongList
		    If song.Left.StringValue.IndexOf(searchText) > -1 Then
		      lst_songs_songs.AddRow song.Left.StringValue
		      lst_songs_songs.CellTag(lst_songs_songs.LastAddedRowIndex, 0) = song.Right.StringValue
		    End If
		  Next
	...
	#tag Method, Flags = &h0
		Function LoadSong(f As FolderItem, updateFolderSelection As Boolean = False) As Boolean
		  ...
		  #If TargetMacOS
		    #Pragma Warning "Figure out what is going on here!"
		    Dim fullpath As String = f.NativePath.Mid(Songs.GetRootFolder().NativePath.Len + 2)
		  #Else
		  ...
		  If f <> Nil And f.Exists Then
		    ...
		    Dim folderPath As String = fullpath.Mid(1, fullpath.Len() - f.Name().Len())
		    folderPath = ReplaceAll(folderPath, "\", "/")
		    If folderPath.Right(1) = "/" Then folderPath = StringUtils.Chop(folderPath, 1)
		    ...
		    If index > -1 Then
		      ...
		      If CurrentSong <> Nil  Then
		        Status_SongOpen = False ' Just to keep the field changes from calling UpdateMenuItems
		        LoadSongFields
		        Status_SongOpen = True
		        Status_SongChanged = False
		        'UpdateMenuItems
		...
		Sub UpdateMenuItems()
		  ...
		  If pge_controls.Value = 0 Then ' Songs tab
		    ...
		    If Status_SongOpen Then
		      ...
		      edt_song_presentation.Enabled = True And (Not Globals.Status_Presentation)
	...
	#tag Property, Flags = &h21
		Private SearchSongList() As Pair
	#tag EndProperty
...
#tag Events sch_songs_songs
	#tag Event
		Sub CancelButtonAction()
		  ClearSongSearch
		End Sub
	#tag EndEvent
	#tag Event
		Function HandleSpecialKey(key as SearchControl.SearchControlSpecialKey) As Boolean
		  If key = SearchControl.SearchControlSpecialKey.EscapeKey Then
		    ClearSongSearch
		  Else
		    DoSongSearch(Me.Text)
		  End If
		End Function
	#tag EndEvent
	#tag Event
...
#tag Events btn_song_show_backgrounds_editor
	...
	#tag Event
		Sub MouseEnter()
		  SetHelp "background_song_editor"
		End Sub
	#tag EndEvent
	#tag Event
		Sub MouseExit()
...
#tag Events chk_song_keepaspect
	#tag Event
		Sub MouseEnter()
		  SetHelp "backgrounds_song_editorkeepaspect"
		End Sub
	#tag EndEvent
	#tag Event
		Sub MouseExit()
...
#tag Events chk_song_fit_to_body
	#tag Event
		Sub MouseEnter()
		  SetHelp "backgrounds_song_editorfit_to_body"
		End Sub
	#tag EndEvent
	#tag Event
		Sub MouseExit()
...
#tag Events chk_song_fit_to_screen
	#tag Event
		Sub MouseEnter()
		  SetHelp "backgrounds_song_editorfit_to_screen"
		End Sub
	#tag EndEvent
	#tag Event
		Sub MouseExit()
...
#tag Events lst_song_backgrounds
	#tag Event
		Sub Change()
		  If (Me.ListIndex() > -1) And (Me.ListIndex < Me.ListCount - 1) Then
	...
	#tag Event
		Function ContextualMenuAction(hitItem as MenuItem) As Boolean
		  ...
		  Try
		    Select Case hitItem.Tag
		    Case App.T.Translate("image_slide/preview/@caption"), "preview"
		      If Me.SelCount = 1 Then
		        For i As Integer = 0 To Me.ListCount - 1
		          If Me.Selected(i) Then
		            img = Me.GetImage(i)
	...
	#tag Event
		Function CellBackgroundPaint(g As Graphics, row As Integer, column As Integer) As Boolean
		  If Me.SelectedRowIndex = row Then Return False
