...
#tag WindowCode
	...
	#tag Event
		Sub Close()
		  ...
		  App.MouseCursor = Nil
		  If hasMainWindow Then MainWindow.Status_Presentation = False
		  Globals.Status_Presentation = False
		  If HelperActive And hasHelperWindow Then PresentHelperWindow.Close
		  ...
		  While slide_group <> Nil
		    If SmartML.GetValue(slide_group, "@type") = "song" Then
		      ...
		      If xSlide Is Nil Then
		        style = GetStyle(SmartML.GetValue(slide_group, "style/@index"))
		      Else
		        style = SetML.GetSlideStyle(xSlide)
		      End If
	...
	#tag Method, Flags = &h0
		Sub ExternalSlideTerminatedW32(lpParameter As Ptr, TimerOrWaitFired As Boolean)
		  ...
		  App.DebugWriter.Write CurrentMethodName + "@@@@@@@@ make me stand out @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
		  'If m_AppLaunchProcess.PID = 0 Then Break
		  App.DebugWriter.Write CurrentMethodName + " still alive"
		  Call m_AppLaunchProcess.IsRunning
		  App.DebugWriter.Write CurrentMethodName + " still alive"
		  ...
		  While slide_group <> Nil
		    ...
		    If SmartML.GetValue(slide_group, "@type", True) = "external" Then
		      ...
		      Case "presentation"
		        ...
		        Dim presFile As FolderItem = GetFolderItem( presFileName, FileUtils.PathType )
		      ...
		      Case "launch"
		        ...
		        Dim appFile As FolderItem = GetFolderItem( appFileName, FileUtils.PathType )
	...
	#tag Method, Flags = &h0
		Sub Present(setDoc As XmlDocument, PresentMode As Integer, Item As Integer = 0)
		  ...
		  CurrentSet.AppendChild CurrentSet.SafeImportNode(setDoc.FirstChild, CopyAllChildren)
		  ...
		  #If RBVersion > 2018.03 Then
		    picWidth = picWidth * OSScreen(PresentScreen).ScaleFactor
		    picHeight = picHeight * OSScreen(PresentScreen).ScaleFactor
		  #EndIf
		  ...
		  If tmpLogo <> Nil Then
		    ...
		    tmpLogo = SmartML.GetValueP(de, "logo_mask")
		    If tmpLogo <> Nil Then
		      #If RBVersion > 2018.03
		        tmpMask = Self.BitmapForCaching(tmpLogo.Width, tmpLogo.Height)
		      #Else
		        tmpMask = New Picture(tmpLogo.Width, tmpLogo.Height, 32)
		      #EndIf
		      tmpMask.Graphics.DrawPicture tmpLogo, _
		      0, 0, tmpMask.Graphics.Width, tmpMask.Graphics.Height, _
		      0, 0, tmpLogo.Width, tmpLogo.Height
		      If tmpMask <> Nil Then
		        LogoCache.ApplyMask tmpMask
		      End If
		    End If
		  End If
	...
	#tag Method, Flags = &h0
		Sub ResetPaint(slide As XmlNode)
		  ...
		  If slide = Nil Then
		    App.DebugWriter.Write CurrentMethodName + " Slide is Nil"
		  Else
		    App.DebugWriter.Write CurrentMethodName + " Slide @type=" + SmartML.GetValue(slide.Parent.Parent, "@type", false) + " @name=" + SmartML.GetValue(slide.Parent.Parent, "@name", False)
		  End If
		  If slide <> Nil Then
		  ...
		  If SetML.IsExternal(PreviousSlide) Then
		    ...
		    ElseIf SetML.IsExternal(slide) Then
		      ...
		      If currAppl <> prevAppl Or _
		        ...
		        action = "stop" Or _
		        SmartML.GetValue(slideGroup, "@_localfilename", False) <> prevFile Or _
		        SmartML.GetValue(slideGroup, "@filename", False) <> prevFile Or _
		        CurrentExternalSlide <> CurrentSlide Then
		        bClose = True
		      End If
		    Else
		    ...
		    If bClose Then
		      ...
		      Select Case prevAppl
		      Case "presentation"
		        Dim presFile As FolderItem = PathToFolderItem( prevFile )
		      ...
		      Case "launch"
		        #If TargetWin32
		          mExternalSlideWait = False
		          App.DebugWriter.Write CurrentMethodName + " closing 'launch' slide, PID=" + CStr(m_AppLaunchProcess.PID)
		          m_AppLaunchProcess.QuitProcess()
		        #Else
		          // Close any running external application instance.
		          mExternalSlideWait = False
		          App.DebugWriter.Write CurrentMethodName + " closing 'launch' slide, PID=" + CStr(m_AppLaunchShell.PID)
		  ...
		  If SetML.IsExternal(slide) Then
		    ...
		    Case "presentation"
		      ...
		      If mode = "N" then
		        ...
		        End If
		        Dim presFile As FolderItem = PathToFolderItem( presFilename )
		        If presFile <> Nil Then
		          If presFile.Exists() Then
		            ...
		            If oExtPres <> Nil Then
		              ...
		              If presIndex <> oExtPres.CurrentSlide() Then
		                ...
		                Else
		                  ...
		                  If slide.PreviousSibling <> Nil Then
		                    Call oExtPres.GotoSlide( presIndex )
		                  End If
		                End If
		              End If
		      ...
		      Else
		        If mode = "N" then
		          ...
		          If CurrentExternalSlide <> CurrentSlide Then
		            ...
		            launchAppLocation = PathToFolderItem(cmd)
		            #If TargetWin32
		            ...
		            If cmd <> "" Or params <> "" Then
		              #If TargetWin32
		                Call m_AppLaunchProcess.CreateProcess(cmd, params, AddressOf PresentWindowLaunchAppExitCallback)
		                App.DebugWriter.Write CurrentMethodName + " 'launch' slide; PID=" + CStr(m_AppLaunchProcess.PID)
		              #Else
		                m_AppLaunchShell.Mode = 1 'Asynchronous
		                m_AppLaunchShell.TimeOut = -1
		                m_AppLaunchShell.Execute(cmd, params)
		                App.DebugWriter.Write CurrentMethodName + " 'launch' slide; PID=" + CStr(m_AppLaunchShell.PID)
		              #Endif
		              CurrentExternalSlide = CurrentSlide
		            End If
		  ...
		  If advanceNext Then
		    App.DebugWriter.Write CurrentMethodName + " perform ACTION_NEXT_SLIDE"
		    Call PerformAction(ACTION_NEXT_SLIDE)
		  End If
		...
		#tag Getter
			Get
			  ...
			  For i As Integer = 1 To XSlides.Length
			    Dim tmp As XmlNode = XSlides.Item(i - 1)
			    If XSlides.Item(i - 1) <> Nil And XSlides.Item(i - 1).XPath = path Then
			      slideNumber = i
			      Exit For
			    End If
			  Next
...
#tag Events cnvSlide
	...
	#tag Event
		Sub Paint(g As Graphics, areas() As REALbasic.Rect)
		  ...
		  If (doTransition And (curslideTransition = SlideTransitionEnum.ApplicationDefault)) _
		    Or (curslideTransition = SlideTransitionEnum.UseTransition) Then
		    ...
		    If inTransition Then
		      inTransition = TransitionFrames > TransitionFrame
		    End If
		  End If
		  If inTransition Then
		  ...
		  Else
		    ...
		    NDI.NDI.SendImage CurrentPicture
		  End If
		  ...
		  Catch ex
		    ...
		    RuntimeException(ex).Message, 1)'MISSING OPENING PARENTHESIS
		    Return
		    #Pragma BreakOnExceptions Default
		End Sub
	#tag EndEvent
