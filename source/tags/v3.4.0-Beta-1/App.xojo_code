...
Inherits Application
	...
	#tag Event
		Sub Open()
		  ...
		  If result <> NO_FOLDER Then
		    ScriptureFolder = DocsFolder.Child(STR_SCRIPTURE)
		    BibleFactory.Folder = ScriptureFolder
		  End If
		  If DocsFolder.Child("OpenSong Scripture").Exists Then
		    BibleFactory.AddSearchFolder DocsFolder.Child("OpenSong Scripture")
		  End If
		  If AppFolder.Child("OpenSong Scripture").Exists Then
		    BibleFactory.AddSearchFolder AppFolder.Child("OpenSong Scripture")
		  End If
		  
		  If Not ScriptureFolder.Exists Or BibleFactory.BibleList.Ubound < 0 Then
		    App.MouseCursor = Nil
		  ...
		  '(as long as the defaults folder isn't empty of course ;)
		  If CheckDefaultFolders(BACKGROUNDS_FOLDER) = FOLDER_EMPTY Then
		    Dim bgfolder As FolderItem
		    bgfolder = App.MainPreferences.GetValueFI(Prefs.kBackgroundsFolder, DocsFolder.Child(STR_BACKGROUNDS), True)
		    If bgfolder <> Nil And Not FileUtils.IsReadonly(bgfolder) And bgfolder.Count = 0 Then
		      App.MouseCursor = Nil
		      If InputBox.AskYN(App.T.Translate("questions/backgrounds_folder_empty/@caption")) Then
		        If Not FileUtils.CopyPath(AppFolder.Child(STR_OS_DEFAULTS).Child(STR_BACKGROUNDS), _
		          App.MainPreferences.GetValueFI(Prefs.kBackgroundsFolder, DocsFolder.Child(STR_BACKGROUNDS), True)) Then'MISSING OPENING PARENTHESIS
		          MsgBox T.Translate("errors/create_backgrounds_folder", _
		          App.MainPreferences.GetValueFI(Prefs.kBackgroundsFolder, DocsFolder.Child(STR_BACKGROUNDS), True).NativePath)
		        End If
		      End If
		    End If
		  ...
		  If  result <> NO_FOLDER Then
		    If NOT Globals.SongActivityLog.Load(DocsFolder.Child("Settings").Child("ActivityLog.xml")) Then
		      MsgBox  T.Translate("errors/activity_disabled", DocsFolder.Child("Settings").Child("ActivityLog.xml").NativePath)
	...
	#tag Event
		Function UnhandledException(error As RuntimeException) As Boolean
		  ...
		  // identify the binary
		  Popup.ErrorDescription.AppendText(EndOfLine + "OpenSong " + App.SmartVersion(True) + "  Build date: " + Str(App.BuildDate))
		  #if TargetMacOS
		    Popup.ErrorDescription.AppendText(" macOS")
		  #elseif TargetWin32
		    Popup.ErrorDescription.AppendText(" Windows")
		  #elseif TargetLinux
		    Popup.ErrorDescription.AppendText(" Linux")
		  #endif
		  ...
		  Popup.ErrorDescription.SelStart = 0
		  Popup.ErrorDescription.ScrollPosition = 0
		  Popup.ErrorDescription.ScrollPositionX = 0
	...
	#tag Method, Flags = &h0
		Function CheckDocumentFolders(which As Integer, Create As Boolean = True, Prompt As Boolean = False) As Integer
		  ...
		  Case BACKGROUNDS_FOLDER
		    Try
		      f = App.MainPreferences.GetValueFI(Prefs.kBackgroundsFolder, DocsFolder.Child(STR_BACKGROUNDS), True)
		    Catch
		    ...
		    If FolderName <> "" Then
		      ...
		      If f = Nil Then
		        ...
		        For Each Folder In Split(FolderName, FileUtils.NativePathSeparator)
		          If f = Nil Then
		            f = PathToFolderItem(Folder, FileUtils.PathType)
		            If f = Nil Then
		              e = New RuntimeException
		              e.Message = "GetDocsFolder: " + App.T.Translate("errors/fileutils/invalidname", FolderName)
		              DebugWriter.Write "GetDocsFolder: Got Nil trying to set root for " + FolderName, 1
		              Raise e
		            End If
		          ElseIf Not FileUtils.CreateFolder(f.Child(Folder)) Then
		            e = New RuntimeException
	...
	#tag Method, Flags = &h0
		Function SmartVersion(WithNonRelease As Boolean = False) As String
		  ...
		  If App.BugVersion > 0 Or WithNonRelease Then
		    t = t + "." + Str(App.BugVersion)
		  End If
