...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub PickMultipleSongs(callingObject As SongPickerReceiver, ShowPresentationOrder As Boolean = True)
		  ...
		  btn_done.Caption = App.T.Translate("shared/done", found)
	...
	#tag Method, Flags = &h0
		Function Popup(ByRef presentation As String, ShowPresentationOrder As Boolean = True) As FolderItem
		  ...
		  edt_order.Enabled = ShowPresentationOrder
		  edt_order.ReadOnly = Not ShowPresentationOrder
		  btn_done.Caption = App.T.Translate("shared/cancel", found)
...
#tag Events edt_quick_lookup
	#tag Event
		Function KeyDown(Key As String) As Boolean
		  ...
		  If HandleShortcutKey(Control(Me), Key) Then
		    Return True
		  End If
	...
	#tag Event
		Sub Change()
		  If Me.ListIndex >= 0 Then
		    ...
		    Else
		      edt_preview.Text = App.T.Translate("errors/bad_format", Me.CellTag(Me.ListIndex, 0).StringValue + Me.List(Me.ListIndex), SmartML.ErrorString)
